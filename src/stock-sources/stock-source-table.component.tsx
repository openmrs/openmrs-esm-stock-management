...
const StockSourceTable: React.FC<StockSourceTableProps> = ({
  ...
  const [startedDeletingStockSource, setStartedDeletingStockSource] =
    useState(false);
  const [deleteStockSource, { reset: resetDeleteStockSourceHook }] =
  ...
  const [canCreateModifyStockSources] = useHasPreviledge(
  ...
  const rows: Array<any> = useMemo(
    () =>
      stockSources?.results?.map((stockSource, index) => ({
        ...
        name: !canCreateModifyStockSources ? (
          `${stockSource?.name}`
        ) : (
          <Link
            to={URL_STOCK_SOURCES}
            onClick={(e) => handlerUserNameClick(e, stockSource?.uuid)}
          >{`${stockSource?.name}`}</Link>
        ),
  ...
  function deleteStockSourceOnContinue() {
    ...
    deleteStockSource(selectedDelete)
      .then((payload: any) => {
        setStartedDeletingStockSource(false);
        if ((payload as any).error) {
          var errorMessage = toErrorMessage(payload);
          errorAlert(
            `${t("stockmanagement.stocksource.delete.failed")} ${errorMessage}`
          );
          return;
        }
              ...
              <TableToolbar>
                ...
                <TableToolbarContent>
                  ...
                  >
                    ...
                    {(stockSourceTypes?.answers &&
                    stockSourceTypes?.answers.length > 0
                      ? stockSourceTypes?.answers
                      : stockSourceTypes?.setMembers
                    )?.map((sourceType) => {
                      return (
                        <SelectItem
                          key={sourceType.uuid}
              ...
              <Table {...getTableProps()}>
                <TableHead>
                  <TableRow>
                    ...
                    {canCreateModifyStockSources && <TableHeader></TableHeader>}
                  </TableRow>
                </TableHead>
                <TableBody>
