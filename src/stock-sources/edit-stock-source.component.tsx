import { Form, Select, SelectItem, SelectSkeleton, TextInput } from 'carbon-components-react';
...
export const EditStockSource: React.FC<EditStockSourceProps> = ({
    ...
    const { data: sourceTypes, error: loadSourceTypesError, isLoading: isLoadingSourceTypes, isFetching: isFetchingSourceTypes, isSuccess: loadedSourceTypes } = useGetConceptByIdQuery(STOCK_SOURCE_TYPE_CODED_CONCEPT_ID);
    ...
    const onSourceTypeChange = (evt: ChangeEvent<HTMLSelectElement>) => {
        let selectedSourceType = (sourceTypes?.answers && sourceTypes?.answers.length > 0 ? sourceTypes?.answers : sourceTypes?.setMembers)?.find(x => x.uuid === evt.target.value);
    ...
    return <Formik innerRef={formikRef} validationSchema={editValidationSchema} initialValues={{
        ...
        {({ errors, touched, validateField, validateForm }) => (
            <Form className='smt-form' onSubmit={onFormSubmit}>
                <TextInput invalid={!!(formikErrors?.name)} invalidText={t2(formikErrors?.name)} id="name" placeholder="e.g. National Medical Stores" name="name" type="text" maxLength={255} defaultValue={formModel?.name} onChange={onNameChanged} labelText={t('stockmanagement.stocksource.edit.name')} />
                <TextInput invalid={!!(formikErrors?.acronym)} invalidText={t2(formikErrors?.acronym)} placeholder="e.g. NMS" id="acronym" name="acronym" type="text" maxLength={255} defaultValue={formModel?.acronym} onChange={onAcronymChanged} labelText={t('stockmanagement.stocksource.edit.acronym')} />
                {
                    isLoadingSourceTypes && <SelectSkeleton hideLabel />
                }
                {
                    !isFetchingSourceTypes && !loadedSourceTypes &&
                    <span className="error-text">{t('stockmanagement.stocksource.loadsourceTypeserror')} {toErrorMessage(loadSourceTypesError)}</span>
                }
                {
                    !isLoadingSourceTypes && loadedSourceTypes &&
                    <Select invalid={!!(formikErrors?.sourceType)} invalidText={t2(formikErrors?.sourceType)} name='sourceType' className='select-field' labelText={t('stockmanagement.stocksource.edit.sourceType')} id="sourceType" value={formModel.sourceType?.uuid ?? "placeholder-item"} onChange={onSourceTypeChange}>
                        <SelectItem disabled hidden value="placeholder-item" text="Choose a source type" />
                        {(sourceTypes?.answers && sourceTypes?.answers.length > 0 ? sourceTypes?.answers : sourceTypes?.setMembers)?.map(sourceType => {
