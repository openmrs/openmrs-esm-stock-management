...
export const ModalPopup: React.FC<ModalPopupProps> = ({
    ...
    const onValidationComplete = async (isSuccess: boolean, updatedModel: StockSource) => {
        setValidateForm(false);
        if (isSuccess) {
            try {
                setShowSplash(true);
                createOrUpdateStockSource(updatedModel).unwrap().then(
                    (payload: any) => {
                        if ((payload as any).error) {
                            var errorMessage = toErrorMessage(payload);
                            errorAlert(`${t(updatedModel.uuid == null ? "stockmanagement.stocksource.createfailed" : "stockmanagement.stocksource.updatefailed")} ${errorMessage}`);
                            return;
                        } else {
                            successAlert(`${t(updatedModel.uuid == null ? "stockmanagement.stocksource.createsuccess" : "stockmanagement.stocksource.updatesuccess")}`);
                            onClose?.(true);
                        }
                    })
                    .catch((error) => {
    ...
    return <>
        ...
        <Modal open={openModal} primaryButtonText={t("stockmanagement.save")}
            ...
            modalHeading={isNew ? t("stockmanagement.stocksource.new.title") : `${model?.name}`}
            onRequestClose={onCancel}
            shouldSubmitOnEnter={false}
            onRequestSubmit={onSave}
            onSecondarySubmit={onCancel}>
            <EditStockSource model={model} isNew={isNew} validateForm={validateForm} onValidationComplete={onValidationComplete} />
        </Modal>
    </>
}