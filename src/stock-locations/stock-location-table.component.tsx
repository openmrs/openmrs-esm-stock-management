...
import React, { CSSProperties, ChangeEvent, useCallback, useMemo, useRef, useState } from 'react';
import { Link } from 'react-router-dom';
import styles from '../../root.module.scss';
import { Splash } from '../components/spinner/Splash';
import { URL_LOCATIONS, URL_LOCATIONS_EDIT, URL_LOCATIONS_NEW } from '../constants';
import { useDeleteLocationMutation, useLazyGetLocationWithIdByUuidQuery } from '../core/api/lookups';
...
const LocationTable: React.FC<LocationTableProps> = ({
  ...
  const [selectedDelete, setSelectedDelete] = useState<OpenMRSLocation | null>(null);
  const [startedDeletingLocation, setStartedDeletingLocation] = useState(false);
  const [locationTag, setLocationTag] = useState<String>();
  const [deleteLocation, { reset: resetDeleteLocationHook }] = useDeleteLocationMutation();
  const [canCreateLocation] = useHasPreviledge([MANAGE_LOCATIONS], false);
  const [getLocationWithIdByUuid, { isFetching: isFetchingLocationWithId }] = useLazyGetLocationWithIdByUuidQuery();
  ...
  const onDeleteLocationClick = useCallback((uuid: string) => {
    if (uuid) {
  ...
  const deleteLocationOnContinue = () => {
    ...
    deleteLocation(selectedDelete.uuid).then(
      (payload: any) => {
        setStartedDeletingLocation(false);
        if ((payload as any).error) {
          var errorMessage = toErrorMessage(payload);
          errorAlert(`${t("stockmanagement.location.delete.failed")} ${errorMessage}`);
          return;
        }
  ...
  const onViewItem = useCallback(async (uuid: string, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    await getLocationWithIdByUuid(uuid).then((payload: any) => {
      if ((payload as any).error) {
  ...
  const onViewItemClick = useCallback((event: React.MouseEvent<HTMLAnchorElement>, uuid: string) => {
    event.preventDefault();
    onViewItem(uuid, null!);
  }, [onViewItem]);

  const rows: Array<any> = useMemo(() =>
    locations?.results?.filter(p => {
      if (locationTag) {
      ...
      uuid: `${location?.uuid}`,
      name: <Link to={URL_LOCATIONS} onClick={(e) => onViewItemClick(e, location.uuid)}>{`${location?.name}`}</Link>,
      tags: location?.tags?.map(p => p.display)?.join(", ") ?? "",
      childLocations: location?.childLocations?.map(p => p.display)?.join(", ") ?? "",
      actions: canCreateLocation ? <div key={`${location?.uuid}-actions`} style={{ display: "inline-block", whiteSpace: "nowrap" }}>
        <Button key={`${location?.uuid}-actions-edit`} type="button" size="sm" className="submitButton clear-padding-margin" iconDescription={"Edit"} kind="ghost" renderIcon={Edit16} onClick={(e) => onViewItem(location.uuid, e)} />
        {(location?.childLocations?.length ?? 0) === 0 && <Button key={`${location?.uuid}-actions-delete`} type="button" size="sm" className="submitButton clear-padding-margin" iconDescription={"Delete"} kind="ghost" renderIcon={TrashCan16} onClick={(e) => onDeleteLocationClick(location.uuid)} />}
      </div> : ""
    })),
    [locations?.results, locationTag, canCreateLocation, onViewItemClick, onViewItem, onDeleteLocationClick],
  );
  ...
  return <>
    ...
    {openDeleteConfirm && <Modal
      ...
      size="sm"
    >
      <p>{t("stockmanagement.location.delete.confirmText")}&nbsp;<b>{selectedDelete?.name}</b>?</p>
    </Modal>
    }
    {(startedDeletingLocation || isFetchingLocationWithId) && <Splash active blockUi />}
          ...
          <TableContainer>
            <TableToolbar>
              <TableToolbarContent>
                ...
                <Select className='stkpg-datable-select' inline id="locationTag" labelText="" onChange={onLocationTagChange}>
                  <SelectItem text='All' value={""} />
                  {locationTags?.results?.map(tag => {
                    return <SelectItem key={tag.uuid} value={tag.uuid} text={tag.display} />
                  })}
                </Select>
                <TableToolbarMenu>
                ...
                {canCreateLocation && <Button onClick={createLocation} size="sm" kind="primary">
