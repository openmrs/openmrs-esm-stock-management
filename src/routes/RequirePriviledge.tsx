...
export const RequirePriviledge = ({ privileges, userContext, requireAllPrivileges, children }: PrivateRouteProps) => {
	const continueToRoute = useMemo(() => {
		return privileges == null ||
			privileges.length === 0 ||
			(userContext?.user?.privileges &&
				(requireAllPrivileges ? hasEveryPrivilegeScope(privileges, (userContext?.user?.privileges)) :
					hasSomePrivilegeScope(privileges, (userContext?.user?.privileges))));
	}, [privileges, userContext, requireAllPrivileges]);
	return <>
		{continueToRoute ? children : <Navigate
			to={{
