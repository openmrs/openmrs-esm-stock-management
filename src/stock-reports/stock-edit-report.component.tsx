import { Checkbox, ComboBox, DatePicker, DatePickerInput, Form, FormGroup, Modal, NumberInput, RadioButton, RadioButtonGroup, RadioButtonValue, Select, SelectItem, SelectSkeleton } from 'carbon-components-react';
...
import { useGetConceptByIdQuery, useLazyGetPatientsQuery } from '../core/api/lookups';
import { StockItemFilter, useLazyGetStockItemsQuery } from '../core/api/stockItem';
...
import { DATE_PICKER_CONTROL_FORMAT, DATE_PICKER_FORMAT, ParseDate, formatDisplayDate, formatForDatePicker } from '../core/utils/datetimeUtils';
import { toErrorMessage } from '../core/utils/stringUtils';
import useTranslation from '../core/utils/translation';
import { ReportParameter, ReportType, getParamDefaultLimit, getReportEndDateLabel, getReportLimitLabel, getReportStartDateLabel } from './stock-report-type';
...
export interface EditReportProps {
    ...
    onCloseEditDialog: (showReportStatus: boolean, batchJobUuid?: string | null) => void;
}
...
export const EditReport: React.FC<EditReportProps> = ({
    ...
    const { data: stockItemCategoriesRemote, isLoading: isLoadingStockItemCategories } = useGetConceptByIdQuery(STOCK_ITEM_CATEGORY_CONCEPT_ID);
    ...
    const [stockItemSearchResult, setStockItemSearchResult] = useState<StockItemDTO[]>([]);
    ...
    const stockItemCategories = useMemo(() => {
        return [{ display: "All Categories", name: "All Categories", uuid: "" } as any as Concept, ...((stockItemCategoriesRemote && stockItemCategoriesRemote?.answers.length > 0 ? stockItemCategoriesRemote?.answers : stockItemCategoriesRemote?.setMembers) ?? [])]
    }, [stockItemCategoriesRemote])

    const stockSources = useMemo(() => {
        return [{ display: "All Sources", name: "All Sources", stockSourceUuid: "" } as any as Party, ...(stockSourceList ?? [])]
    }, [stockSourceList])
    ...
    const onStockItemCategoryChanged = (data: { selectedItem: any }) => {
        ...
        setModel(
            produce((draft) => {
                draft.stockItemCategory = data.selectedItem?.display ?? data.selectedItem?.name;
    ...
    const onStockSourceChanged = (data: { selectedItem: any }) => {
        let newValue = data.selectedItem?.stockSourceUuid ?? data.selectedItem?.uuid;
        setModel(
            produce((draft) => {
                draft.stockSource = data.selectedItem?.display ?? data.selectedItem?.name;
    ...
    const onStockSourceDestinationChanged = (data: { selectedItem: any }) => {
        ...
        setModel(
            produce((draft) => {
                draft.stockSourceDestination = data.selectedItem?.display ?? data.selectedItem?.name;
    ...
    useEffect(() => {
        if (model.stockItemName && model.stockItemUuid) {
            setStockItemSearchResult([{ uuid: model.stockItemUuid, conceptName: model.stockItemName } as any as StockItemDTO])
        }
    ...
    useEffect(() => {
        ...
        if (model.reportSystemName) {
            let reportType = reportTypes?.find(p => p.systemName === model.reportSystemName);
            if (reportType) {
                ...
                setDisplayStockSourceDestination(reportType.parameters?.some(p => p === ReportParameter.StockSourceDestination));
    ...
    const onReportTypeChanged = (data: { selectedItem: any }) => {
        ...
        setModel({ reportName: data.selectedItem?.display ?? data?.selectedItem?.name, reportSystemName: newValue, childLocations: false, limit: getParamDefaultLimit(newValue) });
        let reportType = reportTypes?.find(p => p.systemName === newValue);
        formikRef?.current?.setFieldValue("parameters", reportType?.parameters ?? []);
    ...
    const onInventoryGroupByChange = (evt: ChangeEvent<HTMLSelectElement>) => {
        ...
        setModel(
            produce((draft) => {
                ...
                draft.inventoryGroupByName = newValue === "LocationStockItem" ? t("stockmanagement.report.locationstockitem") :
                    (newValue === "LocationStockItemBatchNo" ? t("stockmanagement.report.locationstockitembatchno") :
                        (newValue === "StockItemOnly" ? t("stockmanagement.report.stockitemonly") : undefined));
            })
        );
        formikRef?.current?.setFieldValue("inventoryGroupBy", newValue);
    }

    const onMostLeastMovingChange = (selection: RadioButtonValue, name: string, evt: ChangeEvent<HTMLInputElement>) => {
        setModel(
            produce((draft) => {
                draft.mostLeastMoving = selection ? selection.toString() : "";
                draft.mostLeastMovingName = selection === "MostMoving" ? t("stockmanagement.report.mostmoving") :
                    (selection === "LeastMoving" ? t("stockmanagement.report.leastmoving") : undefined);
            })
        );
        formikRef?.current?.setFieldValue("mostLeastMoving", selection);
    }

    const onChildLocationsChanged = (cvt: React.ChangeEvent<HTMLInputElement>, data: { checked: boolean; id: string; }): void => {
    ...
    const getReportParameter = (name: string, value: string, valueDescription: string, description: string, newLine: string): string => {
    ...
    const handleSaveBatchJob = useCallback(async (report: ReportModel) => {
        let hideSplash = true;
        try {
            let newLine = "\r\n";
            ...
            if (displayPatient) {
                parameters += getReportParameter(ReportParameter.Patient,
                    report.patientUuid ?? "",
            ...
            if (displayStockItem) {
                parameters += getReportParameter(ReportParameter.StockItem,
            ...
            if (displayStockItemCategory) {
                parameters += getReportParameter(ReportParameter.StockItemCategory,
            ...
            if (displayInventoryGroupBy) {
                parameters += getReportParameter(ReportParameter.InventoryGroupBy,
            ...
            if (displayLocation) {
                parameters += getReportParameter(ReportParameter.Location,
                    report.locationUuid!,
                    report.location?.trim() ?? "",
                    t('stockmanagement.report.edit.location'),
                    newLine);
                if (displayChildLocations) {
                    parameters += getReportParameter(ReportParameter.ChildLocations,
            ...
            if (displayStockSource) {
                parameters += getReportParameter(ReportParameter.StockSource,
            ...
            if (displayStockSourceDestination) {
                parameters += getReportParameter(ReportParameter.StockSourceDestination,
            ...
            if (displayMostLeastMoving) {
                parameters += getReportParameter(ReportParameter.MostLeastMoving,
            ...
            if (displayLimit) {
                parameters += getReportParameter(ReportParameter.Limit,
            ...
            if (displayDate) {
                parameters += getReportParameter(ReportParameter.Date,
            ...
            if (displayStartDate) {
                parameters += getReportParameter(ReportParameter.StartDate,
            ...
            if (displayEndDate) {
                parameters += getReportParameter(ReportParameter.EndDate,
            ...
            await createBatchJob(newItem).unwrap().then(
                (payload: any) => {
                    if ((payload as any).error) {
                        var errorToken = toErrorMessage(payload);
                        errorAlert(`${t("stockmanagement.report.batchjobcreatefailed")} ${errorToken}`);
                        return;
                    } else {
                        successAlert(`${t("stockmanagement.report.batchjobcreatesuccess")}`);
                        refreshBatchJobs();
    ...
    }, [displayFulfillment, displayPatient, displayStockItem, displayStockItemCategory, displayInventoryGroupBy, displayLocation, displayMaxReorderLevelRatio, displayStockSource, displayStockSourceDestination, displayMostLeastMoving, displayLimit, displayDate, displayStartDate, displayEndDate, createBatchJob, setShowSplash, t, displayChildLocations, refreshBatchJobs, onCloseEditDialog]);
    ...
    const handleStockItemsSearch = useMemo(() => debounce((searchTerm) => {
    ...
    const handlePatientsSearch = useMemo(() => debounce((searchTerm) => {
        getPatients({ startIndex: 0, v: ResourceRepresentation.Default, limit: 10, q: searchTerm, totalCount: true, isDrug: null } as any as StockItemFilter)
            .unwrap().then(
                (payload: any) => {
                    if ((payload as any).error) {
                        var errorMessage = toErrorMessage(payload);
    ...
    return <>
        <Modal open={true} primaryButtonText={t("stockmanagement.continue")}
            ...
            <Formik innerRef={formikRef} validationSchema={editValidationSchema} initialValues={{
                ...
                {({ errors, touched, validateField, validateForm, handleSubmit, isSubmitting }) => (
                    <Form className='smt-form' style={{ minHeight: "500px" }} onSubmit={handleSubmit} >
                        <ComboBox titleText={t('stockmanagement.report.edit.reportname')} id="reportSystemName"
                            initialSelectedItem={(model.reportSystemName) ? {
                                systemName: model.reportSystemName,
                                display: model.reportName
                            } as any : null}
                            selectedItem={model.reportSystemName ? {
                                systemName: model.reportSystemName,
                                display: model.reportName
                            } : null}
                            items={model.reportSystemName ? [...(reportTypes.some(x => x.systemName === model.reportSystemName) ? [] : [{ systemName: model.reportSystemName, display: model.reportName }]), ...(reportTypes ?? [])] : reportTypes}
                            onChange={(data: { selectedItem: any }) => onReportTypeChanged(data)}
                            shouldFilterItem={(data) => true}
                            itemToString={item => item?.display ?? item?.name ?? ""}
                            placeholder={'Filter...'}
                            invalid={touched.reportSystemName && !!errors.reportSystemName} invalidText={t2(errors.reportSystemName)}
                        />
                        {model.reportSystemName && <>

                            {
                                ...
                                <div style={{ display: "flex", flexDirection: "row", gap: "0 1rem" }}>
                                    <Checkbox id="allFulfillment" checked={model.fullFillment?.some(x => x === "All") ?? false}
                                        onChange={(checked: boolean, id: string, event: React.ChangeEvent<HTMLInputElement>) => onFullfillmentSelected("All", checked)}
                                        labelText={t('stockmanagement.report.edit.allfullfillment')} />
                                    <Checkbox id="fullFulfillment" checked={model.fullFillment?.some(x => x === "Full") ?? false}
                                        onChange={(checked: boolean, id: string, event: React.ChangeEvent<HTMLInputElement>) => onFullfillmentSelected("Full", checked)}
                                        labelText={t('stockmanagement.report.edit.fullfullfillment')} />
                                    <Checkbox id="partialFulfillment" checked={model.fullFillment?.some(x => x === "Partial") ?? false}
                                        onChange={(checked: boolean, id: string, event: React.ChangeEvent<HTMLInputElement>) => onFullfillmentSelected("Partial", checked)}
                                        labelText={t('stockmanagement.report.edit.partialfullfillment')} />
                                    <Checkbox id="noneFulfillment" checked={model.fullFillment?.some(x => x === "None") ?? false}
                                        onChange={(checked: boolean, id: string, event: React.ChangeEvent<HTMLInputElement>) => onFullfillmentSelected("None", checked)}
                                        labelText={t('stockmanagement.report.edit.nonefullfillment')} />
                                </div>
                            }

                            {
                                displayPatient && <>
                                    <ComboBox titleText={t('stockmanagement.report.edit.patient')} id="cbPatient"
                            ...
                            {
                                displayStockItem && <>
                                    <ComboBox titleText={t('stockmanagement.report.edit.stockitem')} id="cbStockItem"
                                        initialSelectedItem={model?.stockItemUuid && model.stockItemName && stockItemSearchResult?.length === 1 && stockItemSearchResult[0].uuid === model?.stockItemUuid ? stockItemSearchResult[0] : null}
                            ...
                            {displayStockItemCategory && <>
                                {(isLoadingStockItemCategories || !stockItemCategories) && <SelectSkeleton hideLabel />}
                                {
                                    !(isLoadingStockItemCategories || !stockItemCategories) && <>
                                        <ComboBox titleText={t('stockmanagement.report.edit.stockitemcategory')}
                                            invalid={touched.stockItemCategoryConceptUuid && !!errors.stockItemCategoryConceptUuid} invalidText={t2(errors.stockItemCategoryConceptUuid)}
                                            name='stockItemCategoryConceptUuid' className='select-field' id="stockItemCategory"
                                            items={stockItemCategories}
                                            onChange={onStockItemCategoryChanged}
                                            shouldFilterItem={(data) => true}
                                            initialSelectedItem={stockItemCategories?.find(p => p.uuid === (model.stockItemCategoryConceptUuid ?? "")) ?? {} as any}
                                            itemToString={item => item && item?.display ? `${item?.display}` : ""}
                                            placeholder={t("stockmanagement.report.edit.stockitemcategoryholder")} />
                                    </>
                                }
                            </>
                            }
                            {
                                ...
                                <Select id="inventoryGroupBy" defaultValue={model?.inventoryGroupBy}
                                    invalid={touched.inventoryGroupBy && !!errors.inventoryGroupBy} invalidText={t2(errors.inventoryGroupBy)}
                                    labelText={t("stockmanagement.report.edit.inventorygroupby")} onChange={onInventoryGroupByChange}>
                                    <SelectItem value="" text={t("stockmanagement.report.selectinventorygroupby")} />
                                    <SelectItem value="StockItemOnly" text={t("stockmanagement.report.stockitemonly")} />
                                    <SelectItem value="LocationStockItem" text={t("stockmanagement.report.locationstockitem")} />
                                    <SelectItem value="LocationStockItemBatchNo" text={t("stockmanagement.report.locationstockitembatchno")} />
                                </Select>
                            }
                            {displayLocation && <>
                                <ComboBox titleText={t('stockmanagement.report.edit.location')} id="reportLocation"
                                    initialSelectedItem={(model.locationUuid) ? {
                                        uuid: model.locationUuid,
                                        display: model.location
                                    } as any : null}
                                    selectedItem={model.locationUuid ? {
                                        uuid: model.locationUuid,
                                        display: model.location
                                    } : null}
                                    items={model.location ? [...(partyLookupList.some(x => x.locationUuid === model.locationUuid) ? [] : [{ uuid: model.locationUuid, display: model.location }]), ...(partyLookupList ?? [])] : partyLookupList}
                                    onChange={(data: { selectedItem: any }) => onLocationChanged(data)}
                                    shouldFilterItem={(data) => true}
                                    itemToString={item => item?.display ?? item?.name}
                                    placeholder={'Filter...'}
                                    invalid={touched.locationUuid && !!errors.locationUuid} invalidText={t2(errors.locationUuid)}
                                />
                                {
                                    displayChildLocations &&
                                    <Checkbox id="childLocations" checked={model.childLocations ?? false}
                                        onChange={(e: React.ChangeEvent<HTMLInputElement>, d: { checked: boolean; id: string; }) => onChildLocationsChanged(e, d)}
                                        labelText={t('stockmanagement.report.edit.childlocation')} />
                                }
                            </>
                            }
                            {
                                displayMaxReorderLevelRatio &&
                                <NumberInput id="maxReorderLevelRatio" allowEmpty={true}
                                    onChange={(e: any, d: any) => onMaxReorderLevelFieldChange(e?.target?.value)}
                                    value={model?.maxReorderLevelRatio ?? ""} label={t('stockmanagement.report.edit.maxreorderlevelratio')}
                                    invalid={touched.maxReorderLevelRatio && !!errors.maxReorderLevelRatio} invalidText={t2(errors.maxReorderLevelRatio)}
                                />
                            }
                            {displayStockSource && <>
                                <ComboBox titleText={t('stockmanagement.report.edit.stocksource')} id="reportStockSource"
                                    initialSelectedItem={(model.stockSourceUuid) ? {
                                        uuid: model.stockSourceUuid,
                                        display: model.stockSource
                                    } as any : null}
                                    selectedItem={model.stockSourceUuid ? {
                                        uuid: model.stockSourceUuid,
                                        display: model.stockSource
                                    } : null}
                                    items={model.stockSource ? [...(stockSources.some(x => x.stockSourceUuid === model.stockSourceUuid) ? [] : [{ uuid: model.stockSourceUuid, display: model.stockSource }]), ...(stockSources ?? [])] : stockSources}
                                    onChange={(data: { selectedItem: any }) => onStockSourceChanged(data)}
                                    shouldFilterItem={(data) => true}
                                    itemToString={item => item?.display ?? item?.name}
                                    placeholder={'Filter...'}
                                    invalid={touched.stockSourceUuid && !!errors.stockSourceUuid} invalidText={t2(errors.stockSourceUuid)}
                                />
                            </>
                            }
                            {displayStockSourceDestination && <>
                                <ComboBox titleText={t('stockmanagement.report.edit.stocksourcedestination')} id="reportStockSourceDestination"
                                    initialSelectedItem={(model.stockSourceDestinationUuid) ? {
                                        uuid: model.stockSourceDestinationUuid,
                                        display: model.stockSourceDestination
                                    } as any : null}
                                    selectedItem={model.stockSourceDestinationUuid ? {
                                        uuid: model.stockSourceDestinationUuid,
                                        display: model.stockSourceDestination
                                    } : null}
                                    items={model.stockSourceDestination ? [...(stockSources.some(x => x.stockSourceUuid === model.stockSourceDestinationUuid) ? [] : [{ uuid: model.stockSourceDestinationUuid, display: model.stockSourceDestination }]), ...(stockSources ?? [])] : stockSources}
                                    onChange={(data: { selectedItem: any }) => onStockSourceDestinationChanged(data)}
                                    shouldFilterItem={(data) => true}
                                    itemToString={item => item?.display ?? item?.name}
                                    placeholder={'Filter...'}
                                    invalid={touched.stockSourceDestinationUuid && !!errors.stockSourceDestinationUuid} invalidText={t2(errors.stockSourceDestinationUuid)}
                                />
                            </>
                            }
                            <div style={{ display: "grid", gridTemplateColumns: ".5fr .5fr", gap: "0 1rem" }}>
                                {
                                    displayMostLeastMoving &&
                                    <FormGroup className='clear-margin-bottom' legendText={t("stockmanagement.report.edit.mostleastmoving")} title={t("stockmanagement.report.edit.mostleastmoving")}
                                        invalid={touched.mostLeastMoving && !!errors.mostLeastMoving}>
                                        <RadioButtonGroup name="mostLeastMoving"
                                            defaultSelected={model.mostLeastMoving == null ? "" : model.mostLeastMoving}
                                            legendText="" onChange={onMostLeastMovingChange}  >
                                            <RadioButton value="MostMoving" id="mostLeastMovingMost" labelText={t("stockmanagement.report.mostmoving")} />
                                            <RadioButton value="LeastMoving" id="mostLeastMovingLeast" labelText={t("stockmanagement.report.leastmoving")} />
                                        </RadioButtonGroup>
                                        {
                                            touched.mostLeastMoving && !!errors.mostLeastMoving &&
                                            <span className="error-text" style={{ fontSize: "0.75rem" }}>{t2(errors.mostLeastMoving)}</span>
                                        }
                                    </FormGroup>
                                }
                                {
                                    displayLimit &&
                                    <NumberInput id="limitTop" allowEmpty={true}
                                        onChange={(e: any, d: any) => onLimitFieldChange(e?.target?.value)}
                                        value={model?.limit ?? ""} label={t(getReportLimitLabel(model.reportSystemName))}
                                        invalid={touched.limit && !!errors.limit} invalidText={t2(errors.limit)}
                                    />
                                }
                            </div>
                            {
                                displayDate &&
                                <DatePicker datePickerType="single" locale="en" dateFormat={DATE_PICKER_CONTROL_FORMAT} onChange={onDateChange}>
                                    <DatePickerInput invalid={touched.date && !!errors.date} invalidText={t2(errors.date)}
                                        id="date"
                                        name="date"
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: ".5fr .5fr", gap: "0 1rem" }}>
                                {
                                    displayStartDate &&
                                    <DatePicker datePickerType="single" locale="en" dateFormat={DATE_PICKER_CONTROL_FORMAT} onChange={onStartDateChange}>
                                        <DatePickerInput invalid={touched.startDate && !!errors.startDate} invalidText={t2(errors.startDate)}
                                            id="startDate"
                                            name="startDate"
                                            defaultValue={formatForDatePicker(model?.startDate)}
                                            placeholder={DATE_PICKER_FORMAT}
                                            labelText={t(getReportStartDateLabel(model.reportSystemName))}
                                            onChange={(e) => onStartDateChangeInput(e.target.value)}
                                        />
                                    </DatePicker>
                                }
                                {
                                    displayEndDate &&
                                    <DatePicker datePickerType="single" locale="en" dateFormat={DATE_PICKER_CONTROL_FORMAT} onChange={onEndDateChange}>
                                        <DatePickerInput invalid={touched.endDate && !!errors.endDate} invalidText={t2(errors.endDate)}
                                            id="endDate"
                                            name="endDate"
                                            defaultValue={formatForDatePicker(model?.endDate)}
                                            placeholder={DATE_PICKER_FORMAT}
                                            labelText={t(getReportEndDateLabel(model.reportSystemName))}
