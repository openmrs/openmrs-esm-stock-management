...
import { BatchJobFilter, useGetBatchJobsQuery, useGetReportTypesQuery } from '../core/api/batchJob';
...
import { APP_STOCKMANAGEMENT_REPORTS, TASK_STOCKMANAGEMENT_REPORTS_MUTATE } from '../core/privileges';
...
export const ReportList = () => {
    ...
    const [batchJobFilter, setBatchJobFilter] = useState<BatchJobFilter>({
        startIndex: 0, batchJobType: BatchJobTypeReport, v: ResourceRepresentation.Default, limit: 10, q: "", totalCount: true,
    ...
    const { data: batchJobs, isFetching, isLoading, refetch: refetchBatchJobs } = useGetBatchJobsQuery(batchJobFilter, { refetchOnMountOrArgChange: true });
    const { data: reportTypes, isLoading: isLoadingReportTypes } = useGetReportTypesQuery();
    const userPrivilegeScopes = useGetPrivilegeScopes([APP_STOCKMANAGEMENT_REPORTS, TASK_STOCKMANAGEMENT_REPORTS_MUTATE]);
    const { data: partyList, isLoading: isLoadingPartyList } = useGetPartiesQuery();

    useEffect(() => {
        setBatchJobFilter({
            startIndex: currentPage - 1, batchJobType: BatchJobTypeReport, v: ResourceRepresentation.Default, limit: currentPageSize, q: searchString, totalCount: true,
            completedDateMax: completedDateMax,
            completedDateMin: completedDateMin,
            dateCreatedMax: dateCreatedMax,
            dateCreatedMin: dateCreatedMin,
    ...
    }, [searchString, currentPage, currentPageSize, completedDateMax, completedDateMin, dateCreatedMax, dateCreatedMin, locationScopeUuid, batchJobStatus]);

    const handleSearch = useCallback((str) => {
        setSearchString(str ?? "");
    }, []);

    const handleRefetchBatchJobs = () => { refetchBatchJobs(); };

    const gridPartyLookupList = useMemo(() => {
        if (!userPrivilegeScopes || !partyList) {
            return [];
        }
        let locationUuids = userPrivilegeScopes?.filter(p => p.locationUuid && p.privilege === APP_STOCKMANAGEMENT_REPORTS).map(p => p.locationUuid);
    ...
    return (
        <div className="stkpg-page">
            <div className="stkpg-page-header">
                <h1 className="stkpg-page-title">{t('stockmanagement.report.list.title')}</h1>
                <h3 className={`stkpg-page-subtitle ${styles.bodyShort02} ${styles.marginTop} ${styles.whiteSpacePreWrap}`}>{t('stockmanagement.report.list.description')}</h3>
            </div>
                    ...
                    <section>
                        <div className={styles.tableContainer}>
                            <ReportTable
                                ...
                                isLoading={isLoading || isLoadingReportTypes || isLoadingPartyList}
                                isFetching={isFetching}
                                search={{
                                    onSearch: handleSearch,
                                    refetch: handleRefetchBatchJobs,
                                    onCompletedDateChange: (a, b) => { setCompletedDateMax(b); setCompletedDateMin(a); },
                                    onDateCreatedChange: (a, b) => { setDateCreatedMax(b); setDateCreatedMin(a); },
                                ...
                                pagination={{
                                    ...
                                    pagesUnknown: false,
                                    lastPage: (batchJobs?.results?.length || 0) < currentPageSize || ((currentPage * currentPageSize) === batchJobs?.totalCount),
                                }}
                            />
                        </div>
                    </section>
