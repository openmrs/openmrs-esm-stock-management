...
export const EditUserScope: React.FC<EditUserScopeProps> = ({
  ...
  const [
    getUsers,
    { data: users, error: loadUsersError, isLoading: isLoadingUsers },
  ] = useLazyGetUsersQuery();
  const [
    getUser,
    {
  ...
  const {
    ...
    isSuccess: loadedLocations,
  } = useGetLocationsQuery({ v: ResourceRepresentation.Default });
  const {
    data: stockOperationTypes,
    error: loadStockOperationTypesError,
    isFetching: isFetchingStockOperationTypes,
    isSuccess: loadedStockOperationTypes,
  } = useGetStockOperationTypesQuery();
  ...
  const onRoleChange = (e: ChangeEvent<HTMLSelectElement>) => {
    let rootLocations = locations?.results
  ...
  const onStockOperationTypeChanged = (
    ...
    let operationType = formModel.operationTypes.find(
      (x) => x.operationTypeUuid === cvt.target.value
    );
    if (operationType) {
      let newOperationTypes = [
      ...
      formikRef?.current?.setFieldValue(
        "operationType",
        newOperationTypes.map((x) => x.operationTypeUuid)
      );
    } else {
      let stockOperationType = stockOperationTypes?.results?.find(
      ...
      formikRef?.current?.setFieldValue("operationType", [
        stockOperationType?.uuid,
      ]);
    }
  ...
  const onLocationCheckBoxChanged = (
    ...
    let selectedLocation = formModel.locations?.find(
    ...
    if (selectedLocation) {
      ...
      formikRef?.current?.setFieldValue(
        "location",
        newLocations.map((x) => x.locationUuid)
      );
    } else {
  ...
  const findCheckedLocation = (
    ...
    let result = formModel.locations?.filter(
  ...
  return (
    <Formik
      ...
      initialValues={{
        ...
        operationType: formModel?.operationTypes?.map(
        ...
        <Form className="user-role-scope-edit smt-form" onSubmit={onFormSubmit}>
          ...
          {isNew && !isLoadingUsers && loadUsersError && (
            <span className="error-text">
              {t("stockmanagement.useruserscope.loaduserserror")}{" "}
              {toErrorMessage(loadUsersError)}
            </span>
          )}
          {isNew && !isLoadingUsers && (
            <>
              <ComboBox
                titleText={t("stockmanagement.userrolescope.edit.user")}
                invalid={!!formikErrors?.user}
                invalidText={t(formikErrors?.user)}
                name="user"
                id="select-user"
                light
                items={(users?.results ?? []).filter(
                  (x) => x.uuid !== currentUserId
                )}
          ...
          {!isNew && !isFetchingUser && !loadedUser && (
            <span className="error-text">
              {t("stockmanagement.userrolescope.loadroleserror")}{" "}
              {toErrorMessage(loadUserError)}
            </span>
          )}

          <Select
            invalid={!!formikErrors?.role}
            invalidText={t2(formikErrors?.role)}
            name="role"
          ...
          <div className="user-role-scope-permanent">
            <Checkbox
              ...
              labelText={t("stockmanagement.userrolescope.list.header.enabled")}
            />
            <Checkbox
              id="chk-permanent"
              name="isPermanent"
              ...
              labelText={t(
                "stockmanagement.userrolescope.list.header.permanent"
              )}
            />
            {!formModel.permanent && (
              <>
                ...
                >
                  <DatePickerInput
                    invalid={!!formikErrors?.activeFrom}
                    invalidText={t2(formikErrors?.activeFrom)}
                    id="date-picker-input-id-start"
                    name="activeFrom"
                    placeholder={DATE_PICKER_FORMAT}
                    labelText={t(
                      "stockmanagement.userrolescope.list.header.activeFrom"
                    )}
                    value={formatForDatePicker(formModel.activeFrom)}
                  />
                  <DatePickerInput
                    invalid={!!formikErrors?.activeTo}
                    invalidText={t2(formikErrors?.activeTo)}
                    id="date-picker-input-id-finish"
                    name="activeTo"
                    placeholder={DATE_PICKER_FORMAT}
                    labelText={t(
                      "stockmanagement.userrolescope.list.header.activeTo"
                    )}
          ...
          {!isFetchingStockOperationTypes && !loadedStockOperationTypes && (
            <span className="error-text">
              {t("stockmanagement.loadstockoperationtypeserror")}{" "}
              {toErrorMessage(loadStockOperationTypesError)}
            </span>
          )}
          {!isFetchingStockOperationTypes && loadedStockOperationTypes && (
            <>
              <div className="field-label">
                <span className="field-title underline">
                  {t("stockmanagement.userrolescope.edit.stockoperations")}
                </span>
                <span className="field-desc">
                  {t(
                    "stockmanagement.userrolescope.edit.stockoperationsdescription"
                  )}
                </span>
                {formikErrors?.operationType && (
                  <div className="error-msg">
                    {t2(formikErrors?.operationType)}
                  </div>
                )}
              </div>
              <div className="operation-types">
                {sortedOperationTypes.map((stockOperationType) => {
                  return (
                    <div
                      key={`chk-sopt-child-key-${stockOperationType.uuid}`}
                      className="operation-type"
                    >
                      <Checkbox
                        name="operationType"
                        id={`chk-sopt-child-${stockOperationType.uuid}`}
                        value={stockOperationType.uuid}
                        onChange={onStockOperationTypeChanged}
                        checked={isOperationChecked(stockOperationType)}
                        labelText={stockOperationType.name}
                      />
                    </div>
                  );
                })}
          ...
          {!isFetchingLocations && !loadedLocations && (
            <span className="error-text">
              {t("stockmanagement.userrolescope.loadlocationserror")}{" "}
              {toErrorMessage(loadLocationsError)}
            </span>
          )}
          {!isLoadingLocations && loadedLocations && (
            <>
              <div className="field-label">
                <span className="field-title underline">
                  {t("stockmanagement.userrolescope.edit.locations")}
                </span>
                <span className="field-desc">
                  {t(
                    "stockmanagement.userrolescope.edit.locationsdescendantsdescription"
                  )}
                </span>
                {formikErrors?.location && (
                  <div className="error-msg">{t2(errors?.location)}</div>
                )}
              </div>
              <div className="locations">
                {sortedLocations
                  .filter(
                    (x) =>
                      formModel.role === INVENTORY_ROLE_NAME ||
                      !!x.parentLocation
                  )
                  .map((location) => {
                    ...
                    return (
                      <div
                        key={`div-loc-child-key-${location.uuid}`}
                        className="location"
                      >
                        <Checkbox
                          name="location"
                          key={`chk-loc-child-key-${location.uuid}`}
                          id={`chk-loc-child-${location.uuid}`}
                          onChange={onLocationCheckBoxChanged}
                          value={location.uuid}
                          checked={checkedLocation != null}
                          labelText={location.display}
                        />
                        {checkedLocation && (
                          <ToggleSmall
                            aria-label={t(
                              "stockmanagement.userrolescope.locationsbelow"
                            )}
                            toggled={
                              checkedLocation?.enableDescendants === true
                            }
                            onChange={onEnableDescendantsChanged}
                            value={location.uuid}
                            key={`tg-loc-child-key-${location.uuid}`}
                            id={`tg-loc-child-${location.uuid}`}
                          />
                        )}
                      </div>
                    );
