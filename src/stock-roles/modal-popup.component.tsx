...
export const ModalPopup: React.FC<ModalPopupProps> = ({
    ...
    const onValidationComplete = async (isSuccess: boolean, updatedModel: UserRoleScope) => {
    ...
    return <>
        ...
        <Modal open={openModal} size="lg" primaryButtonText={t("stockmanagement.save")}
            ...
            modalHeading={isNew ? t("stockmanagement.userrolescope.new.title") : `${model?.userFamilyName} ${model?.userGivenName}`}
            onRequestClose={onCancel}
            shouldSubmitOnEnter={false}
            onRequestSubmit={onSave}
            onSecondarySubmit={onCancel}>
            <EditUserScope model={model} isNew={isNew} validateForm={validateForm} onValidationComplete={onValidationComplete} />
        </Modal>
    </>
}