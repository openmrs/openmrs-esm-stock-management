...
const UserRoleScopeTable: React.FC<UserRoleScopeTableProps> = ({
  ...
  const [startedDeletingUserRoleScope, setStartedDeletingUserRoleScope] =
    useState(false);
  const [deleteUserRoleScope, { reset: resetDeleteUserRoleScopeHook }] =
    useDeleteUserRoleScopesMutation();
  const currentUserId = useAppSelector(selectUserId);
  const handlerUserNameClick = useCallback(
  ...
  const [canCreateModifyUserRoleScopes] = useHasPreviledge(
  ...
  const rows: Array<any> = useMemo(
    () =>
      userRoleScopes?.results?.map((userRoleScope, index) => ({
        ...
        user:
          currentUserId === userRoleScope.userUuid ||
          !canCreateModifyUserRoleScopes ? (
            ...
            <Link
              ...
              onClick={(e) => handlerUserNameClick(e, userRoleScope?.uuid)}
          ...
          return (
            <span key={key}>
              ...
              {location?.enableDescendants ? (
                <ArrowDownLeft16 key={`${key}-${index}-0`} />
              ) : (
                <ArrowLeft16 key={`${key}-${index}-1`} />
              )}{" "}
            </span>
          );
          //return
  ...
  function deleteUserRoleScopeOnContinue() {
    ...
    deleteUserRoleScope(selectedDelete)
      .then((payload: any) => {
        ...
        if ((payload as any).error) {
          ...
          errorAlert(
            `${t(
              "stockmanagement.userrolescope.delete.failed"
            )} ${errorMessage}`
          );
          return;
        }
      ...
      .catch((error) => {
        ...
        errorAlert(
          `${t("stockmanagement.userrolescope.delete.failed")} ${errorMessage}`
        );
        return;
      });
  }
