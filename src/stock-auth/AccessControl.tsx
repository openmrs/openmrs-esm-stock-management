...
export const AccessControl = ({
  ...
  const displayChildren = useMemo(() => {
    return (
      ...
      (userPrivileges &&
        (requireAllPrivileges
          ? hasEveryPrivilegeScope(privileges, userPrivileges)
...
export const useHasPreviledge = (
  privileges: string[],
  requireAllPrivileges: boolean
) => {
  const userPrivileges = useAppSelector(selectPrivileges);
  const displayChildren = useMemo(() => {
    return (
  ...
  const privilegeScopes = useMemo(() => {
    ...
    return (
      userPrivilegeScopes?.filter(
        (p) =>
          privileges.includes(p.privilege) &&
          (p.isPermanent ||
            (p.activeFrom &&
              p.activeTo &&
              now.getTime() >= p.activeFrom.getTime() &&
              now.getTime() <= p.activeTo.getTime()))
      ) ?? []
    );
