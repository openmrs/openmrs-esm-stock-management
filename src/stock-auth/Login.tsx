...
const parseAccounts = function (
  accounts: undefined | null | string
): LoginAccount[] | null {
  if (accounts == null) return null;
  const loginAccounts = accounts
    .split(",")
    .map((account) => {
...
const LoginAccounts: React.FC<{
  ...
  const [accountList, setAccountList] = useState<Array<LoginAccount> | null>(
    null
  );
...
const LoginForm = () => {
  ...
  const handleLoginAccountSelection = (
    ...
    <>
      <div style={{ padding: 50, margin: "0 auto", width: "50%" }}>
        <form onSubmit={handleSubmit}>
          ...
          {process.env.REACT_APP_DEV_USERS && (
            <ul>
              <LoginAccounts
                accounts={process.env.REACT_APP_DEV_USERS}
                onSelectAccount={handleLoginAccountSelection}
              />
            </ul>
          )}
...
export const Login = () => {
  ...
  return <>{displayLogin ? <LoginForm /> : <Splash active={true} />}</>;
};
