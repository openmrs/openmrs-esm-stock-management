...
const StockItemTable: React.FC<StockItemTableProps> = ({
  ...
  const [startedDeletingStockItem, setStartedDeletingStockItem] =
    useState(false);
  const [deleteStockItem, { reset: resetDeleteStockItemHook }] =
  ...
  //   editStockItem(uuid);
  // }, [editStockItem]);
  const [canCreateModifyStockItems] = useHasPreviledge(
  ...
  const headers = [
    ...
    // },
    {
      ...
      header: t(
        "stockmanagement.stockitem.list.header.defaultStockOperationsUoMName"
      ),
    },
    // {
    //   key: 'retired',
    ...
    () =>
      stockItems?.results?.map((stockItem, index) => ({
        ...
        type: stockItem?.drugUuid
          ? t("stockmanagement.drug")
          : t("stockmanagement.other"),
        genericname: (
        ...
        retired: stockItem?.voided
          ? t("stockmanagement.yes")
          : t("stockmanagement.no"),
        isdrug: stockItem?.drugUuid
          ? t("stockmanagement.yes")
          : t("stockmanagement.no"),
          ...
          <div className="tbl-expand-display-fields">
            <div className="field-label">
              <span className="field-title">
                {t("stockmanagement.stockitem.edit.commonname")}
              </span>
            ...
            <div className="field-label">
              <span className="field-title">
                {t("stockmanagement.stockitem.edit.abbreviation")}
              </span>
              <span className="field-desc">
                {stockItem.acronym ?? t("stockmanagement.na")}
              </span>
            </div>
            {stockItem.drugUuid != null && (
              <div className="field-label">
                <span className="field-title">
                  {t("stockmanagement.stockitem.edit.dispensingunit")}
                </span>
                <span className="field-desc">
                  {stockItem.dispensingUnitPackagingUoMName ??
                    t("stockmanagement.na")}
                </span>
              </div>
            )}
            ...
            {stockItem.reorderLevel && (
              <div className="field-label">
                <span className="field-title">
                  {t("stockmanagement.stockitem.edit.reorderlevel")}
                </span>
                <span className="field-desc">
                  {stockItem?.reorderLevel?.toLocaleString()}{" "}
                  {stockItem.reorderLevelUoMName}
            ...
            {stockItem.purchasePrice && (
              <div className="field-label">
                <span className="field-title">
                  {t("stockmanagement.stockitem.edit.purchaseprice")}
                </span>
                <span className="field-desc">
                  {stockItem?.purchasePrice?.toLocaleString()}/
                  {stockItem.purchasePriceUoMName}
                </span>
              </div>
            )}
          </div>
  ...
  function deleteStockItemOnContinue() {
    ...
    deleteStockItem(selectedDelete)
      .then((payload: any) => {
        setStartedDeletingStockItem(false);
        if ((payload as any).error) {
          var errorMessage = toErrorMessage(payload);
          errorAlert(
            `${t("stockmanagement.stockitem.delete.failed")} ${errorMessage}`
          );
          return;
        }
              ...
              <TableToolbar>
                ...
                <TableToolbarContent>
                  ...
                  <RadioButtonGroup
                    ...
                    defaultSelected={search.currentItemType}
                    name="is-drug"
                    onChange={handleIsDrugChange}
                  >
                    <RadioButton
                      value=""
                      labelText={t(
