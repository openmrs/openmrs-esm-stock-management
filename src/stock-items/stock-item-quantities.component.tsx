...
const StockItemQuantitiesTable: React.FC<StockItemQuantitiesTableProps> = ({
  ...
  const [stockItemInventoryFilter, setStockItemInventoryFilter] = useState<StockItemInventoryFilter>({ startIndex: 0, v: ResourceRepresentation.Default, limit: 10, q: null, totalCount: true, stockItemUuid: stockItemUuid, groupBy: 'LocationStockItem', totalBy: 'StockItemOnly' });
  const { data: stockItemQuantities, isLoading: isLoadingQuantities, isUninitialized: transactionUnitialised } = useGetStockItemInventoryQuery(stockItemInventoryFilter, { refetchOnMountOrArgChange: true });

  useEffect(() => {
    setStockItemInventoryFilter({ startIndex: currentPage - 1, v: ResourceRepresentation.Default, limit: currentPageSize, partyUuid: partyUuid, totalCount: true, stockItemUuid: stockItemUuid, groupBy: 'LocationStockItem', totalBy: 'StockItemOnly' });
  }, [currentPage, currentPageSize, partyUuid, stockItemUuid]);

  const headers = [
    { key: 'location', header: t('stockmanagement.stockitem.batchinfo.location'), styles: { width: "30%" } },
    { key: 'quantity', header: t('stockmanagement.stockitem.batchinfo.quantity'), styles: { width: "10%" } },
    { key: 'quantityuom', header: t('stockmanagement.stockitem.batchinfo.quantityuom'), styles: { width: "20%", whiteSpace: "nowrap" } },
  ];
