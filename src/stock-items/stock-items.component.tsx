...
export const StockItems: React.FC<StockItemsProps> = ({ userContext }) => {
  ...
  return (
    <div className="row">
      <div className="col-12">
        <Routes>
          <Route
            key="redirect-stock-item-route"
            path={"/redirect/:id"}
            element={
              <RedirectToStockItem key={`redir-${currentLocation.key}`} />
            }
          />
          ,
          <Route
            key="new-stock-items-route"
            path={"/new"}
            element={
              <RequirePriviledge
              ...
              >
                <Edit key={`new-${currentLocation.key}`} />
              </RequirePriviledge>
            }
          />
          <Route
            key="edit-stock-items-route"
            path={"/:id"}
            element={
              <RequirePriviledge
                ...
                userContext={userContext}
              >
                <Edit key={`edit-${currentLocation.key}`} />
              </RequirePriviledge>
            }
          />
          <Route
            key="list-stock-items-route"
            path={"/"}
            element={
              <RequirePriviledge
                requireAllPrivileges={true}
          ...
          />
          <Route
            key="default-stock-items-route"
            path={"*"}
            element={<Navigate to={URL_STOCK_ITEMS} />}
          />
          ,
        </Routes>
      </div>
