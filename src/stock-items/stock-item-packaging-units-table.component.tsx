...
interface StockOperationItemTableProps {
  ...
  canEdit: boolean;
  setPackagingUnits: React.Dispatch<
    React.SetStateAction<StockItemPackagingUOMDTO[]>
  >;
  actions: {
    onGoBack: () => void;
    onSave: () => void;
...
const StockOperationItemsTable: React.FC<StockOperationItemTableProps> = ({
  ...
  const [getConcept, { data: concept, isFetching: isFetchingConcept }] =
  ...
  useEffect(() => {
    async function loadLookups() {
      if (canEdit) {
        if (!concept) {
          await getConcept(PACKAGING_UNITS_CODED_CONCEPT_ID)
            .unwrap()
            .then((payload: any) => {
              ...
              setconceptPackagingUnits(
                pkgUnits?.answers && pkgUnits?.answers.length > 0
                  ? pkgUnits?.answers
                  : pkgUnits?.setMembers
              );
            })
  ...
  const headers = [
    {
      key: "packingunit",
      header: t("stockmanagement.stockitem.units.packingunit"),
      styles: { width: "20%" },
    },
    {
      key: "quantity",
      header: t("stockmanagement.stockitem.units.factor"),
      styles: { width: "10%" },
    },
  ];
  ...
  if (
    canEdit &&
    (isFetchingConcept ||
      !conceptPackagingUnits ||
      conceptPackagingUnits.length === 0)
  ) {
    return (
      <DataTableSkeleton
                            ...
                            <>
                              <ComboBox
                                ...
                                items={
                                  row?.packagingUomUuid
                                    ? [
                                        ...(conceptPackagingUnits.some(
                                          (x) =>
                                            x.uuid === row?.packagingUomUuid
                                        )
                                          ? []
                                          : [
                                              {
                                ...
                                onChange={(data: { selectedItem: any }) =>
                                  onPackagingUnitChanged(row, data)
                                }
                                ...
                                invalid={
                                  row.uuid in errors &&
                                  "packagingUomUuid" in errors[row.uuid] &&
                                  !errors[row.uuid]["packagingUomUuid"]
                                }
                              />
                            </>
                          )}
                          ...
                          {canEdit && (
                            <NumberInput
                              ...
                              onChange={(e: any, d: any) =>
                                onFactorFieldChange(row, e?.target?.value)
                              }
                              value={row?.factor ?? ""}
                              title=""
                              invalidText=""
                              invalid={
                                row.uuid in errors &&
                                "factor" in errors[row.uuid] &&
                                !errors[row.uuid]["factor"]
                              }
                            />
                          )}
