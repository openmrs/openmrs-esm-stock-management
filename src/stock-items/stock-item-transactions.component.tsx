...
import { StockItemTransactionFilter, useGetStockItemTransactionsQuery } from '../core/api/stockItem';
...
const StockItemTransactionsTable: React.FC<StockItemTransactionsTableProps> = ({
  ...
  const [stockItemTransactionFilter, setStockItemTransactionFilter] = useState<StockItemTransactionFilter>({ startIndex: 0, v: ResourceRepresentation.Default, limit: 10, q: null, totalCount: true, stockItemUuid: stockItemUuid });
  const { data: stockItemTransactions, isLoading: isLoadingTransactions, isUninitialized: transactionUnitialised } = useGetStockItemTransactionsQuery(stockItemTransactionFilter, { refetchOnMountOrArgChange: true });
  const [transactionDateMin, setTransactionDateMin] = useState<string | null | undefined>(null);
  const [transactionDateMax, setTransactionDateMax] = useState<string | null | undefined>(null);
  useEffect(() => {
    setStockItemTransactionFilter({
      startIndex: currentPage - 1,
      v: ResourceRepresentation.Default, limit: currentPageSize,
      partyUuid: partyUuid, totalCount: true, stockItemUuid: stockItemUuid,
      dateMin: transactionDateMin, dateMax: transactionDateMax

    });
  }, [currentPage, currentPageSize, partyUuid, stockItemUuid, transactionDateMax, transactionDateMin]);

  const headers = [
    { key: 'date', header: t('stockmanagement.stockitem.transactions.datecreated') },
    { key: 'party', header: t('stockmanagement.stockitem.transactions.location') },
    { key: 'transactiontype', header: t('stockmanagement.stockitem.transactions.transactiontype') },
    { key: 'qtyin', header: t('stockmanagement.stockitem.transactions.quantityin') },
    { key: 'qtyout', header: t('stockmanagement.stockitem.transactions.quantityout') },
    { key: 'batchno', header: t('stockmanagement.stockitem.transactions.batchno') },
    { key: 'reference', header: t('stockmanagement.stockitem.transactions.reference') },
    { key: 'status', header: t('stockmanagement.stockitem.transactions.status') }
  ];
  ...
  const rows: Array<any> = useMemo(() =>
    stockItemTransactions?.results?.map((stockItemTransaction, index) => ({
      ...
      party: stockItemTransaction.operationSourcePartyName && stockItemTransaction.operationDestinationPartyName ?
        ((stockItemTransaction.operationSourcePartyName === stockItemTransaction?.partyName) ? (
          (stockItemTransaction.quantity > 0 ? <><span className='transaction-location'>{stockItemTransaction.operationSourcePartyName}</span> <ArrowLeft16 /> {stockItemTransaction.operationDestinationPartyName}</> : <><span className='transaction-location'>{stockItemTransaction.operationSourcePartyName}</span> <ArrowRight16 /> {stockItemTransaction.operationDestinationPartyName}</>)
        ) : (stockItemTransaction.operationDestinationPartyName === stockItemTransaction?.partyName) ?

          (stockItemTransaction.quantity > 0 ? <><span className='transaction-location'>{stockItemTransaction.operationDestinationPartyName}</span> <ArrowLeft16 /> {stockItemTransaction.operationSourcePartyName}</> : <><span className='transaction-location'>{stockItemTransaction.operationDestinationPartyName}</span> <ArrowRight16 /> {stockItemTransaction.operationSourcePartyName}</>)
      ...
      qtyout: stockItemTransaction?.quantity < 0 ? `${(-1 * stockItemTransaction?.quantity)?.toLocaleString()} ${stockItemTransaction?.packagingUomName ?? ""}` : ""
    })) ?? [],
  ...
  const onActiveDatesChange = (dates: Date[]): void => {
    setTransactionDateMax(dates[1] ? JSON.stringify(dates[1].setHours(23, 59, 59, 999)).replaceAll("\"", "") : null);
    setTransactionDateMin(dates[0] ? JSON.stringify(dates[0]).replaceAll("\"", "") : null);
  }
  ...
  return <>
    <div className={styles.tableOverride}>
      ...
      <Pagination
        ...
        pagesUnknown={false}
        isLastPage={(stockItemTransactions?.results?.length || 0) < currentPageSize || ((currentPage * currentPageSize) === stockItemTransactions?.totalCount)}
