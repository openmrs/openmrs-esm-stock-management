...
import { URL_STOCK_HOME, URL_STOCK_ITEM, URL_STOCK_ITEMS, URL_STOCK_ITEMS_NEW } from '../constants';
...
const StockItemList = () => {
  ...
  const [stockItemFilter, setStockItemFilter] = useState<StockItemFilter>({ startIndex: 0, v: ResourceRepresentation.Default, limit: 10, q: null, totalCount: true, });
  const [currentPage, setPageCount] = useState(1);
  const [currentPageSize, setCurrentPageSize] = useState(10);
  const [searchString, setSearchString] = useState(null);
  useEffect(() => {
    setStockItemFilter({ startIndex: currentPage - 1, v: ResourceRepresentation.Default, limit: currentPageSize, q: searchString, totalCount: true, isDrug: isDrug });
  }, [searchString, currentPage, currentPageSize, isDrug]);
  const { data: stockItems, isFetching, isLoading, refetch: refetchStockItems } = useGetStockItemsQuery(stockItemFilter, { refetchOnMountOrArgChange: true });
  ...
  const handleRefetchStockItems = () => { refetchStockItems(); };

  return (
    <div className="stkpg-page">
      <div className="stkpg-page-header">
        <h1 className="stkpg-page-title">{t('stockmanagement.stockitem.list.title')}</h1>
        <h3 className={`stkpg-page-subtitle ${styles.bodyShort02} ${styles.marginTop} ${styles.whiteSpacePreWrap}`}>{t('stockmanagement.stockitem.list.description')}</h3>
      </div>
            ...
            <div className={styles.tableContainer}>
              <StockItemListTable
                ...
                pagination={{
                  ...
                  pagesUnknown: false,
                  lastPage: (stockItems?.results?.length || 0) < currentPageSize || ((currentPage * currentPageSize) === stockItems?.totalCount),
                }}
              />
            </div>
          </section>
