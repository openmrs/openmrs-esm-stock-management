...
export interface ConfirmModalPopupProps {
    ...
    onConfirm: (confirmType: string, reason: string | null, confirmParam?: string) => void;
    showModal: boolean;
    confirmParam?: string;
}
...
export const ConfirmModalPopup: React.FC<ConfirmModalPopupProps> = ({
    ...
    return <>
        <Modal size='sm' open={openModal} primaryButtonText={t("stockmanagement.yes")}
            ...
            {
                requireReason && <TextArea id="confirmReason" name="confirmReason"
                value={reason ?? ""}
                invalid={reasonInvalid} invalidText={t("stockmanagement.field.required")}
