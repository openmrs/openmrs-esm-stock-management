...
describe('Receipt Stock Operation step 2 (stock operation items details)', () => {
  beforeEach(() => {
    ...
    (useStockItem as jest.Mock).mockReturnValue({
      isLoading: false,
      error: undefined,
      item: { commonName: 'mock-name', uuid: 'mock-uuid' } as StockItemDTO,
    });
  });
  ...
  it('should properly handle stock operation item selection', async () => {
    (useStockItems as jest.Mock).mockReturnValue({
      stockItemsList: [{ uuid: 'mock-uuid', commonName: 'mock-common-name' }] as Array<StockItemDTO>,
      setLimit: jest.fn(),
      setRepresentation: jest.fn(),
      isLoading: false,
  ...
  it('should render stock operation items in data table', async () => {
    ...
    (useFormContext as jest.Mock).mockReturnValue({
      watch: jest.fn().mockReturnValue([
        {
          uuid: 'mock-uuid',
          stockItemPackagingUOMUuid: 'mock-uom-uuid',
          quantity: mockQuantity,
          stockItemUuid: 'mock-item-uuid',
          batchNo: '12',
          expiration: new Date(),
          stockBatchUuid: 'bacth-mock-uuid',
          purchasePrice: 300,
        },
    ...
    expect(useStockItem).toBeCalledWith('mock-item-uuid');
    expect(screen.getByText('mock-stock-item-common name')).toBeInTheDocument(); // Find by name
  });
