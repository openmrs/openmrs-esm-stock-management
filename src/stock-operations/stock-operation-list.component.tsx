...
import { ALLOW_STOCK_ISSUE_WITHOUT_REQUISITION, STOCK_SOURCE_TYPE_CODED_CONCEPT_ID, URL_STOCK_HOME, URL_STOCK_OPERATION, URL_STOCK_OPERATIONS, URL_STOCK_OPERATIONS_NEW } from '../constants';
import { ResourceRepresentation } from '../core/api/api';
import { useGetConceptByIdQuery, useGetStockOperationTypesQuery, useLazyGetPartiesQuery } from '../core/api/lookups';
import { StockItemFilter, useLazyGetStockItemsQuery } from '../core/api/stockItem';
import { StockOperationFilter, useGetStockOperationsQuery } from '../core/api/stockOperation';
...
import { StockOperationType, StockOperationTypeIsStockIssue } from '../core/api/types/stockOperation/StockOperationType';
...
const StockOperationList = () => {
  ...
  const [operationTypeUuid, setOperationTypeUuid] = useState<string | null | undefined>(null);
  const [sourceTypeUuid, setSourceTypeUuid] = useState<string | null | undefined>(null);
  const [locationUuid, setLocationUuid] = useState<string | null | undefined>(null);
  const [stockItemUuid, setStockItemUuid] = useState<string | null | undefined>(null);
  const [operationDateMin, setOperationDateMin] = useState<string | null | undefined>(null);
  const [operationDateMax, setOperationDateMax] = useState<string | null | undefined>(null);
  const [isLocationOther, setIsLocationOther] = useState<Boolean | null | undefined>(null);
  ...
  const { data: sourceTypes } = useGetConceptByIdQuery(STOCK_SOURCE_TYPE_CODED_CONCEPT_ID);
  ...
  const [stockOperationFilter, setStockOperationFilter] = useState<StockOperationFilter>({
    startIndex: 0, v: ResourceRepresentation.Default, limit: 10, q: null, totalCount: true,
    status: status,
  ...
  useEffect(() => {
    setStockOperationFilter({
      startIndex: currentPage - 1, v: ResourceRepresentation.Default, limit: currentPageSize, q: searchString, totalCount: true,
      status: status,
      operationTypeUuid: operationTypeUuid,
      locationUuid: locationUuid,
      stockItemUuid: stockItemUuid,
  ...
  }, [searchString, currentPage, currentPageSize, status, operationTypeUuid, locationUuid, stockItemUuid, operationDateMin, operationDateMax, isLocationOther, sourceTypeUuid]);
  const { data: stockOperations, isFetching, isLoading, refetch: refetchStockOperations } = useGetStockOperationsQuery(stockOperationFilter, { refetchOnMountOrArgChange: true });
  ...
  const filterOperationTypes = useMemo(() => {
    ...
    let applicablePrivilegeScopes = userPrivilegeScopes?.map(p => p.operationTypeUuid);
    if (applicablePrivilegeScopes && applicablePrivilegeScopes.length > 0) {
      ...
      result = stockOperationTypes?.results?.filter(p => applicablePrivilegeScopes?.includes(p.uuid));
    }
  ...
  const createOperationTypes = useMemo(() => {
    let result: StockOperationType[] | undefined = [];
    let applicablePrivilegeScopes = userPrivilegeScopes?.filter(p => p.privilege === TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE)?.map(p => p.operationTypeUuid);
    if (applicablePrivilegeScopes && applicablePrivilegeScopes.length > 0) {
      result = stockOperationTypes?.results?.filter(p => applicablePrivilegeScopes?.includes(p.uuid));
    }
    if (result && result.length > 0 && allParties && allParties.results.length > 0) {
      let partyList: Party[] = allParties.results;
      result = result.filter(currentStockOperationType => {
        if (StockOperationTypeIsStockIssue(currentStockOperationType.operationType) && !ALLOW_STOCK_ISSUE_WITHOUT_REQUISITION) {
          return false;
        }
        let operationTypePrivileges = userPrivilegeScopes?.filter(p => p.privilege === TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE && p.operationTypeUuid === currentStockOperationType?.uuid);
        ...
        if (currentStockOperationType?.hasSource && currentStockOperationType?.sourceType === LocationTypeLocation) {
          ...
          requiredDestinations = operationTypePrivileges.map(p => p.locationUuid!);
        }
  ...
  const handleRefetchStockOperations = () => { refetchStockOperations(); };
  return (
    <div className="stkpg-page">
      <div className="stkpg-page-header stkpg-opts-items-header">
        <div>
          <h1 className="stkpg-page-title">{t('stockmanagement.stockoperation.list.title')}</h1>
          <h3 className={`stkpg-page-subtitle ${styles.bodyShort02} ${styles.marginTop} ${styles.whiteSpacePreWrap}`}>{t('stockmanagement.stockoperation.list.description')}</h3>
        </div>
        <div className="stock-item-filter">
          <ComboBox titleText="Stock Item" id="cbStockItem"
            ...
            <div className={styles.tableContainer}>
              <StockOperationListTable
                ...
                pagination={{
                  ...
                  pagesUnknown: true,
                  lastPage: (stockOperations?.results?.length || 0) < currentPageSize || ((currentPage * currentPageSize) === stockOperations?.totalCount),
                }}
              />
            </div>
          </section>
