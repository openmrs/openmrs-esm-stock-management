...
export const SubmitApproval: React.FC<SubmitApprovalProps> = ({
  ...
  return (
    <div className="smt-form">
      {canEdit && !locked && (
        <FormGroup
          legendText={t("stockmanagement.stockoperation.edit.approvalrequired")}
          title={t("stockmanagement.stockoperation.edit.approvalrequired")}
        >
          <RadioButtonGroup
            name="rbgApprovelRequired"
            legendText="asdasd asd asd "
            onChange={onApprovalRequiredChange}
            defaultSelected={
              model.approvalRequired == null
                ? ""
                : model.approvalRequired.toString().toLowerCase()
            }
          >
            <RadioButton
              value="true"
      ...
      {!canEdit && (
        <>
          <TextInput
            ...
            readOnly={true}
            labelText={t(
              "stockmanagement.stockoperation.edit.approvalrequired"
            )}
          />
        </>
      )}
          ...
          {model.approvalRequired != null && (
            <>
              ...
              {requiresDispatchAcknowledgement && !model.approvalRequired && (
                ...
                >
                  {t("stockmanagement.stockoperation.edit.dispatch")}
                </Button>
              )}
              {model.approvalRequired && (
                <Button
                  ...
                  renderIcon={SendFilled24}
                >
                  {t("stockmanagement.stockoperation.edit.submit")}
                </Button>
              )}
            </>
          )}
