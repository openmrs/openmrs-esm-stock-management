...
import { StockItemFilter, useLazyGetStockBatchesQuery, useLazyGetStockItemInventoryQuery, useLazyGetStockItemPackagingUOMsQuery, useLazyGetStockItemsQuery } from '../core/api/stockItem';
...
import { DATE_PICKER_CONTROL_FORMAT, DATE_PICKER_FORMAT, ParseDate, formatForDatePicker, today } from '../core/utils/datetimeUtils';
                                ...
                                {items.map((row: any, rowIndex) => {
                                    return (
                                        <TableRow
                                            ...
                                            {canEdit && <TableCell>
                                                <Button type="button" size="sm" className="submitButton clear-padding-margin" iconDescription={"Delete"} kind="ghost" renderIcon={16} onClick={(e) => onRemoveItem(row, e)} />
                                            </TableCell>}
                                        </TableRow>
                                    )
                                })}
