import { Arrival24, CheckmarkOutline24, CloseOutline24, DeliveryTruck24, Departure24, Error24, OverflowMenuVertical24, Printer24, Repeat24 } from '@carbon/icons-react';
...
import { STOCK_OPERATION_PRINT_DISABLE_BALANCE_ON_HAND, STOCK_OPERATION_PRINT_DISABLE_COSTS, URL_STOCK_HOME, URL_STOCK_OPERATION, URL_STOCK_OPERATIONS, URL_STOCK_OPERATIONS_NEW_OPERATION, URL_STOCK_OPERATIONS_REDIRECT } from '../constants';
import { ResourceRepresentation } from '../core/api/api';
import { useLazyGetPartiesQuery, useLazyGetStockOperationTypesQuery } from '../core/api/lookups';
import { StockItemInventoryFilter, useLazyGetStockItemInventoryQuery, useLazyGetStockOperationItemsCostQuery } from '../core/api/stockItem';
import { useCreateStockOperationMutation, useDeleteStockOperationItemMutation, useExecuteStockOperationActionMutation, useLazyGetStockOperationAndItemsQuery, useLazyGetStockOperationLinksQuery, useUpdateStockOperationBatchNumbersMutation, useUpdateStockOperationMutation } from '../core/api/stockOperation';
...
import { StockOperationActionLineItem, StopOperationAction, StopOperationActionType } from '../core/api/types/stockOperation/StockOperationAction';
import { StockOperationDTO } from '../core/api/types/stockOperation/StockOperationDTO';
import { StockOperationItemCost } from '../core/api/types/stockOperation/StockOperationItemCost';
import { StockOperationItemDTO } from '../core/api/types/stockOperation/StockOperationItemDTO';
import { StockOperationStatusCancelled, StockOperationStatusNew, StockOperationStatusRejected, StockOperationStatusReturned } from '../core/api/types/stockOperation/StockOperationStatus';
import { REQUISITION_OPERATION_TYPE, STOCK_ISSUE_OPERATION_TYPE, StockOperationPrintHasItemCosts, StockOperationType, StockOperationTypeCanBeRelatedToRequisition, StockOperationTypeCanCapturePurchasePrice, StockOperationTypeHasPrint, StockOperationTypeIsNegativeQtyAllowed, StockOperationTypeIsQuantityOptional, StockOperationTypeIsReceipt, StockOperationTypeIsRequistion, StockOperationTypeIsTransferOut, StockOperationTypeRequiresActualBatchInformation, StockOperationTypeRequiresBatchUuid, StockOperationTypeRequiresDispatchAcknowledgement, StockOperationTypeRequiresStockAdjustmentReason } from '../core/api/types/stockOperation/StockOperationType';
import { MAIN_STORE_LOCATION_TAG } from '../core/consts';
import { APP_STOCKMANAGEMENT_STOCKOPERATIONS, TASK_STOCKMANAGEMENT_STOCKOPERATIONS_APPROVE, TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE } from '../core/privileges';
import { errorAlert, errorMessage, successAlert } from '../core/utils/alert';
import { BreadCrumbs } from '../core/utils/breadCrumbs';
import { formatDisplayDateTime, isDateAfterToday, today } from '../core/utils/datetimeUtils';
import { getStockOperationUniqueId, isNullOrEmptryOrWhiteSpace, toErrorMessage } from '../core/utils/stringUtils';
...
export const StockOperationsEdit = () => {
  ...
  const [editableModel, setEditableMode] = useState<StockOperationDTO>({ operationDate: today() } as unknown as StockOperationDTO);
  ...
  const [isRequisitionAndCanIssueStock, setIsRequisitionAndCanIssueStock] = useState(false);
  ...
  const [stockOperationType, setStockOperationType] = useState<StockOperationType | null | undefined>(null);
  ...
  const [getStockOperationLinks, { data: stockOperationLinks, isSuccess: fetchedStockOperationLinks }] = useLazyGetStockOperationLinksQuery();
  const [getPartyList] = useLazyGetPartiesQuery();
  const userPrivilegeScopes = useGetPrivilegeScopes([TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE, APP_STOCKMANAGEMENT_STOCKOPERATIONS, TASK_STOCKMANAGEMENT_STOCKOPERATIONS_APPROVE]);
  const navigate = useNavigate();
  const [stockOperationItems, setStockOperationItems] = useState<StockOperationItemDTO[]>([{ uuid: `new-item-1`, id: `new-item-1` }]);
  const [selectedTab, setSelectedTab] = useState(0);
  const [isNegativeQtyAllowed, setIsNegativeQtyAllowed] = useState(false);
  const [allowExpiredBatchNumbers, setAllowExpiredBatchNumbers] = useState(false);
  const [requiresBatchUuid, setRequiresBatchUuid] = useState(false);
  const [requiresActualBatchInformation, setRequiresActualBatchInformation] = useState(false);
  const [requireStockAdjustmentReason, setRequireStockAdjustmentReason] = useState(false);
  ...
  const [receivedItemValidity, setReceivedItemValidity] = useState<{ [key: string]: { [key: string]: boolean } }>({});
  ...
  useEffect(() => {
    async function loadStockOperation() {
      ...
      setShowLoading(true);
      let isNewOperation = null === id || id === undefined || id.indexOf('-') < 0;
      setIsNew(isNewOperation);
      let stockOperation: StockOperationDTO = { operationDate: today() } as unknown as StockOperationDTO;
      if (!isNewOperation) {
        let failedToLoadStockOperation = false;
        await getStockOperation(id!, false).unwrap()
          .then((payload: any) => {
            ...
            if (stockOperation && stockOperation.stockOperationItems) {
              ...
              stockOperation.stockOperationItems.forEach((si, ii, iar) => {
                ...
                if (si.quantityRequested && si.quantityRequestedPackagingUOMName) {
      ...
      await getStockOperationTypes().unwrap()
        .then((payload: any) => {
          if ((payload as any).error) {
            handleErrors(payload);
            navigate(URL_STOCK_OPERATIONS);
          ...
          stockOperationTypes = (payload as PageableResult<StockOperationType>).results;
        })
        .catch((error: any) => handleErrors(error));

      let currentStockOperationType = stockOperationTypes?.find(p => isNewOperation ? p.operationType === id : p.uuid === stockOperation?.operationTypeUuid);
      setStockOperationType(currentStockOperationType);
      setIsNegativeQtyAllowed(StockOperationTypeIsNegativeQtyAllowed(currentStockOperationType?.operationType ?? ""));
      setRequiresBatchUuid(StockOperationTypeRequiresBatchUuid(currentStockOperationType?.operationType ?? ""));
      setRequiresActualBatchInformation(StockOperationTypeRequiresActualBatchInformation(currentStockOperationType?.operationType ?? ""));
      setIsQuantityOptional(StockOperationTypeIsQuantityOptional(currentStockOperationType?.operationType ?? ""));
      setCanCapturePurchasePrice(StockOperationTypeCanCapturePurchasePrice(currentStockOperationType?.operationType ?? ""));
      setRequireStockAdjustmentReason(StockOperationTypeRequiresStockAdjustmentReason(currentStockOperationType?.operationType ?? ""))
      setRequiresDispatchAcknowledgement(StockOperationTypeRequiresDispatchAcknowledgement(currentStockOperationType?.operationType ?? ""))
      setAllowExpiredBatchNumbers(currentStockOperationType?.allowExpiredBatchNumbers ?? false);
      if (isNewOperation) {
        stockOperation = { operationDate: today(), operationTypeName: currentStockOperationType?.name, operationTypeUuid: currentStockOperationType?.uuid, operationType: currentStockOperationType?.operationType } as any as StockOperationDTO;
        if (StockOperationTypeCanBeRelatedToRequisition(currentStockOperationType?.operationType ?? "")) {
          try {
            requisition = urlQueryParams.get("requisition");
            if (requisition) {
              await getStockOperation(requisition, false).unwrap()
                .then((payload: any) => {
                  if ((payload as any).error) {
                    handleErrors(payload);
                    return;
                  }
                  let requisitionStockOperation = cloneDeep(payload) as StockOperationDTO;
                  if (!requisitionStockOperation.responsiblePersonUuid && requisitionStockOperation.responsiblePersonOther) {
                    requisitionStockOperation.responsiblePersonUuid = "Other";
                  }
                  let newItemsToCopy: StockOperationItemDTO[] = [];
                  if (requisitionStockOperation && requisitionStockOperation.stockOperationItems) {
                    let hasQtyRequested = false;
                    requisitionStockOperation.stockOperationItems.forEach((si, ii, iar) => {
                  ...
                  stockOperation.destinationUuid = requisitionStockOperation.sourceUuid;
                  stockOperation.destinationName = requisitionStockOperation.sourceName;
                  stockOperation.responsiblePersonUuid = requisitionStockOperation.responsiblePersonUuid;
                  stockOperation.responsiblePersonOther = requisitionStockOperation.responsiblePersonOther;
                  stockOperation.responsiblePersonFamilyName = requisitionStockOperation.responsiblePersonFamilyName;
                  stockOperation.responsiblePersonGivenName = requisitionStockOperation.responsiblePersonGivenName;
      ...
      if (isNewOperation) {
        applicablePrivilegeScopes = userPrivilegeScopes?.filter(p => p.privilege === TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE && p.operationTypeUuid === currentStockOperationType?.uuid);
        canEditModel = applicablePrivilegeScopes?.length > 0;
        setCanEdit(canEditModel);
        if (!canEditModel) {
          errorAlert(t("stockmanagement.stockoperation.nomutateprivilegescopes"));
          navigate(URL_STOCK_OPERATIONS);
          return;
        }
      } else {
      ...
      if (canEditModel) {
        ...
        applicablePrivilegeScopes = applicablePrivilegeScopes ?? userPrivilegeScopes?.filter(p => p.privilege === TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE && p.operationTypeUuid === currentStockOperationType?.uuid);
        if (applicablePrivilegeScopes.length === 0) {
          errorAlert(t("stockmanagement.stockoperation.nomutateprivilegescopes"));
          navigate(URL_STOCK_OPERATIONS);
          return;
        }
        let sourceTags = currentStockOperationType?.stockOperationTypeLocationScopes?.filter(p => currentStockOperationType?.hasSource && p.isSource).map(p => p.locationTag) ?? [];
        let toLockSource = sourceTags.length === 1 && sourceTags[0] === MAIN_STORE_LOCATION_TAG;
        let destinationTags = currentStockOperationType?.stockOperationTypeLocationScopes?.filter(p => currentStockOperationType?.hasDestination && p.isDestination).map(p => p.locationTag) ?? [];
        let toLockDestination = destinationTags.length === 1 && destinationTags[0] === MAIN_STORE_LOCATION_TAG;

        let requiredSources: string[] | null = null;
        let requiredDestinations: string[] | null = null;
        if (currentStockOperationType?.hasSource && currentStockOperationType?.sourceType === LocationTypeLocation) {
          requiredSources = applicablePrivilegeScopes.map(p => p.locationUuid!);
        } else if (currentStockOperationType?.hasDestination && currentStockOperationType?.destinationType === LocationTypeLocation) {
          requiredDestinations = applicablePrivilegeScopes.map(p => p.locationUuid!);
        }
        if (requiredSources == null && requiredDestinations == null) {
          errorAlert(t("stockmanagement.stockoperation.operationtypelocationtyperequired"));
          navigate(URL_STOCK_OPERATIONS);
          return;
        }

        if (currentStockOperationType?.hasSource) {
          let result = partyList?.filter(p => (p.locationUuid && currentStockOperationType?.sourceType === LocationTypeLocation &&
          ...
          if (requiredSources) {
            result = result.filter(p => requiredSources?.includes(p.locationUuid));
            if (result.length === 0) {
              errorAlert(t("stockmanagement.stockoperation.operationtypelocationtagsnomatch"));
              navigate(URL_STOCK_OPERATIONS);
              return;
            }
          }
        ...
        if (currentStockOperationType?.hasDestination) {
          let result = partyList?.filter(p => (p.locationUuid && currentStockOperationType?.destinationType === LocationTypeLocation &&
          ...
          if (requiredDestinations) {
            result = result.filter(p => requiredDestinations?.includes(p.locationUuid));
            if (result.length === 0) {
              errorAlert(t("stockmanagement.stockoperation.operationtypelocationtagsnomatch"));
              navigate(URL_STOCK_OPERATIONS);
              return;
      ...
      if (!isNewOperation) {
        ...
        if (stockOperation.stockOperationItems && stockOperation.stockOperationItems.length > 0) {
          setHasItems(true);
          setShowItems(true);
      ...
      if ((isNewOperation && requisition) || (!isNewOperation &&
        (StockOperationTypeCanBeRelatedToRequisition(currentStockOperationType?.operationType!) ||
          REQUISITION_OPERATION_TYPE === currentStockOperationType?.operationType))) {
  ...
  const isStockOperationItemEmpty = useCallback((itemDto: StockOperationItemDTO): boolean => {
    if (itemDto == null) return true;
    if (itemDto.stockItemUuid) return false;
    if (requiresActualBatchInformation && (itemDto.batchNo || itemDto.expiration))
      return false;
    else if (requiresBatchUuid && itemDto.stockBatchUuid)
      return false;
  ...
  const validateReceivedItems = useCallback(() => {
    ...
    if (!validItems) {
      errorAlert(t("stockmanagement.stockoperation.edit.receiveditemsnotvalid"));
    }
  ...
  const validateItems = useCallback(() => {
    ...
    stockOperationItems.forEach(p => {
      ...
      if (!isQuantityOptional) {
        ...
        if (p.quantity && !p.stockItemPackagingUOMUuid) {
          validationStatus[p.uuid!]["stockItemPackagingUOMUuid"] = false;
          validItems = false;
        } else if (p.stockItemPackagingUOMUuid && (!p.quantity || (!isNegativeQtyAllowed && p.quantity < 0))) {
          validationStatus[p.uuid!]["quantity"] = false;
          validItems = false;
        }
      }
    ...
    return validItems;
  }, [canCapturePurchasePrice, isNegativeQtyAllowed, isQuantityOptional, isStockOperationItemEmpty, requiresActualBatchInformation, requiresBatchUuid, stockOperationItems, t])
  ...
  const getUpdateStockOperationModel = useCallback((): any => {
    let stockOperation = {
      ...
      responsiblePersonUuid: editableModel.responsiblePersonUuid === "Other" ? null : editableModel.responsiblePersonUuid,
    ...
    if (isNew) {
      ...
      stockOperation["requisitionStockOperationUuid"] = StockOperationTypeCanBeRelatedToRequisition(stockOperationType?.operationType!) ? requisitionUuid : null;
    }

    let items: any[] = []
    stockOperationItems.forEach(p => {
      if (p.uuid?.startsWith("new-item") && isStockOperationItemEmpty(p)) return;
      let newItem = {
      ...
      if (!p.uuid?.startsWith("new-item")) {
        newItem["uuid"] = p.uuid;
      } else if (StockOperationTypeCanBeRelatedToRequisition(stockOperationType?.operationType ?? "") && requisitionUuid) {
        newItem["quantityRequested"] = p.quantityRequested;
        newItem["quantityRequestedPackagingUOMUuid"] = p.quantityRequestedPackagingUOMUuid;
      }
      items.push(newItem)
    });
  ...
  const completeDispatch = useCallback(async () => {
    ...
    try {
      ...
      if (saveSuccess) {
        await executeStockOperationAction({ name: 'COMPLETE', uuid: editableModel?.uuid! }).unwrap().then(
          (submitPayload: any) => {
            if (submitPayload && (submitPayload as any).error) {
              var errorToken = toErrorMessage(submitPayload);
              errorMessage(`${t("stockmanagement.stockoperation.completefailed")} ${errorToken}`);
              return;
            }
            else {
              successAlert(`${t("stockmanagement.stockoperation.completesuccess")}`);
            }
          })
          .catch((error) => {
            var errorToken = toErrorMessage(error);
  ...
  }, [editableModel?.uuid, executeStockOperationAction, navigate, selectedTab, t, validateReceivedItems]);
  ...
  }, [editableModel?.uuid, executeStockOperationAction, stockOperationItems, t, validateReceivedItems]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await (isNew ? createStockOperation(stockOperation) : updateStockOperation({ model: stockOperation, uuid: editableModel.uuid! })).unwrap().then(
        (payload: any) => {
          if ((payload as any).error) {
            var errorToken = toErrorMessage(payload);
            errorAlert(`${t(editableModel.uuid == null ? "stockmanagement.stockoperation.createfailed" : "stockmanagement.stockoperation.updatefailed")} ${errorToken}`);
            return;
          } else {

            setShowSplash(false);
            hideSplash = false;
            successAlert(`${t(editableModel.uuid == null ? "stockmanagement.stockoperation.createsuccess" : "stockmanagement.stockoperation.updatesuccess")}`);
            if (isNew) {
              navigate(URL_STOCK_OPERATION((payload as StockOperationDTO).uuid!));
            } else {
              //setShowSplash(true);
              //window.location.reload();
              navigate(URL_STOCK_OPERATIONS_REDIRECT((payload as StockOperationDTO).uuid!, selectedTab?.toString() ?? ""));
            }
          }
        })
        .catch((error) => {
  ...
  const handleCompleteDispatch = useCallback(() => {
    ...
    setConfirmHeading("stockmanagement.stockoperation.confirm.complete.heading");
  ...
  const handleDispatch = useCallback(() => {
    ...
    setConfirmHeading("stockmanagement.stockoperation.confirm.dispatch.heading");
  ...
  const handleRemoveItem = useCallback((itemDto: StockOperationItemDTO) => {
    let text = t("stockmanagement.stockoperation.confirm.removeitem.text");
    text = text.replaceAll("%item.name%", itemDto?.stockItemName ?? "");
    setConfirmText(text);
    setConfirmHeading("stockmanagement.stockoperation.confirm.removeitem.heading");
  ...
  const onConfirmRemoveItem = async (itemUuid: string | undefined | null): Promise<void> => {
    if (!itemUuid) {
      return;
    }
    setShowSplash(true);
    try {
      await deleteStockOperationItem(itemUuid).unwrap().then(
        (payload: any) => {
          if (payload && (payload as any).error) {
            var errorToken = toErrorMessage(payload);
            errorAlert(`${t("stockmanagement.stockoperation.deletefailed")} ${errorToken}`);
            return;
          } else {
            successAlert(`${t("stockmanagement.stockoperation.deletesuccess")}`);
  ...
  const onConfirmSubmit = async (complete: boolean, isDispatch: boolean): Promise<void> => {
    setShowSplash(true);
    let hideSplash = true;
    try {
      let stockOperation = getUpdateStockOperationModel();
      let stockOperationUuid: string | null | undefined = isNew ? null : editableModel.uuid;
      ...
      if (saveSuccess) {
        await executeStockOperationAction({ name: complete ? 'COMPLETE' : (isDispatch ? "DISPATCH" : 'SUBMIT'), uuid: stockOperationUuid! }).unwrap().then(
          (submitPayload: any) => {
            if (submitPayload && (submitPayload as any).error) {
              var errorToken = toErrorMessage(submitPayload);
              errorMessage(`${t(complete ?
                "stockmanagement.stockoperation.completefailed" :
                (isDispatch ? "stockmanagement.stockoperation.dispatchfailed" : "stockmanagement.stockoperation.submitfailed")
              )} ${errorToken}`);
              return;
            }
            else {
              successAlert(`${t(complete ?
                "stockmanagement.stockoperation.completesuccess" :
                (isDispatch ? "stockmanagement.stockoperation.dispatchsuccess" : "stockmanagement.stockoperation.submitsuccess")
              )}`);
            }
          })
          .catch((error) => {
        ...
        if (isNew) {
          ...
          navigate(URL_STOCK_OPERATIONS_REDIRECT(stockOperationUuid!, selectedTab?.toString() ?? ""));
        }
      }
    } finally {
      if (hideSplash) {
  ...
  const executeOperationAction = async (confirmType: string, uuid: string, reason?: string | null) => {
    setShowSplash(true);
    let hideSplash = true;
    ...
    try {
      ...
      switch (confirmType) {
        case "cancel": actionName = "CANCEL"; break;
        case "reject": actionName = "REJECT"; break;
        case "return": actionName = "RETURN"; break;
        case "approve": actionName = "APPROVE"; break;
      ...
      await executeStockOperationAction({ name: actionName, uuid: uuid, reason: reason }).unwrap().then(
        (confirmPayload: any) => {
          if (confirmPayload && (confirmPayload as any).error) {
            var errorToken = toErrorMessage(confirmPayload);
            errorMessage(`${t(`stockmanagement.stockoperation.${messagePrefix}failed`)} ${errorToken}`);
            return;
          }
          else {
            successAlert(`${t(`stockmanagement.stockoperation.${messagePrefix}success`)}`);
          }
        })
        .catch((error) => {
          var errorToken = toErrorMessage(error);
      ...
      setShowSplash(false);
      hideSplash = false;
      navigate(URL_STOCK_OPERATIONS_REDIRECT(uuid, selectedTab?.toString() ?? ""));
  ...
  const handleIssueStock = useCallback(() => {
    navigate(URL_STOCK_OPERATIONS_NEW_OPERATION(STOCK_ISSUE_OPERATION_TYPE, editableModel?.uuid!))
  ...
  const onPrintStockOperation = async () => {
    ...
    try {
      ...
      if (editableModel.requisitionStockOperationUuid) {
        await getStockOperation(editableModel.requisitionStockOperationUuid, false).unwrap()
          .then((payload: any) => {
            if ((payload as any).error) {
              handleErrors(payload);
              return;
      ...
      if (parentOperation || StockOperationPrintHasItemCosts(editableModel.operationType!) || StockOperationTypeIsTransferOut(editableModel.operationType!)) {
        let enableOperationPrintCosts = !STOCK_OPERATION_PRINT_DISABLE_COSTS;
        if (enableOperationPrintCosts) {
          await getStockOperationItemsCost(editableModel.uuid!, false).unwrap()
            .then((payload: any) => {
              if ((payload as any).error) {
                handleErrors(payload);
                return;
      ...
      if (enableBalance && (parentOperation || StockOperationTypeIsRequistion(editableModel.operationType!))) {
        ...
        inventoryFilter.date = JSON.stringify(parentOperation?.dateCreated ?? editableModel?.dateCreated).replaceAll("\"", "");
        await getStockItemInventory(inventoryFilter).unwrap()
          .then((payload: any) => {
            if ((payload as any).error) {
              handleErrors(payload);
              return;
      ...
      let data = await BuildStockOperationData(editableModel, stockOperationItems, parentOperation!, itemsCost, itemInventory);
      if (data) {
        if (StockOperationTypeIsReceipt(editableModel?.operationType!)) {
          await PrintGoodsReceivedNoteStockOperation(data);
        } else if (StockOperationTypeIsTransferOut(editableModel?.operationType!)) {
  ...
  return <>
    ...
    <div className="stkpg-page">
      <div className="stkpg-page-body stkpg-clear-overflow skpg-tab-panel-no-horiz-padding">
        {
          <div className={`stkpg-operation-approval ${!canEdit && !canApprove && !canReceiveItems && !isRequisitionAndCanIssueStock && !canPrint ? "stkpg-operation-approval-hide" : ""}`}>
            <div className='edit-expand-display-fields'>
              {editableModel?.status && <div className='field-label operation-status'>
                <span className='field-title'>{t('stockmanagement.status')}:</span>
                <span className={`field-desc ${editableModel?.status}`}>{editableModel?.status}</span>
              </div>
              }
              {editableModel?.dateCreated && <div className='field-label'>
                <span className='field-title'>{t('stockmanagement.started')}:</span>
                <span className='field-desc'>
                  <span className='action-date'>{formatDisplayDateTime(editableModel?.dateCreated)}</span> {t('stockmanagement.by')} <span className='action-by'>{editableModel.creatorFamilyName ?? ""} {editableModel.creatorGivenName ?? ""}</span>
                </span>
              </div>
              }
              {
                editableModel?.status !== StockOperationStatusNew && editableModel?.status !== StockOperationStatusReturned && editableModel?.submittedDate &&
                <div className='field-label'>
                  <span className='field-title'>{t('stockmanagement.submitted')}:</span>
                  <span className='field-desc'>
                    <span className='action-date'>{formatDisplayDateTime(editableModel?.submittedDate)}</span> {t('stockmanagement.by')} <span className='action-by'>{editableModel.submittedByFamilyName ?? ""} {editableModel.submittedByGivenName ?? ""}</span>
                  </span>
                </div>
              }

              {
                ...
                <div className='field-label'>
                  <span className='field-title'>{t('stockmanagement.dispatched')}:</span>
                  <span className='field-desc'>
                    <span className='action-date'>{formatDisplayDateTime(editableModel?.dispatchedDate)}</span> {t('stockmanagement.by')} <span className='action-by'>{editableModel.dispatchedByFamilyName ?? ""} {editableModel.dispatchedByGivenName ?? ""}</span>
                  </span>
                </div>
              }
              {
                ...
                <div className='field-label'>
                  <span className='field-title'>{t('stockmanagement.returned')}:</span>
                  <span className='field-desc'>
                    <span className='action-date'>{formatDisplayDateTime(editableModel?.returnedDate)}</span> {t('stockmanagement.by')} <span className='action-by'>{editableModel.returnedByFamilyName ?? ""} {editableModel.returnedByGivenName ?? ""}</span>
...

              {
                ...
                <div className='field-label'>
                  <span className='field-title'>{t('stockmanagement.completed')}:</span>
                  <span className='field-desc'>
                    <span className='action-date'>{formatDisplayDateTime(editableModel?.completedDate)}</span> {t('stockmanagement.by')} <span className='action-by'>{editableModel.completedByFamilyName ?? ""} {editableModel.completedByGivenName ?? ""}</span>
                  </span>
                </div>
              }

              {
                ...
                <div className='field-label'>
                  <span className='field-title'>{t('stockmanagement.cancelled')}:</span>
                  <span className='field-desc'>
                    <span className='action-date'>{formatDisplayDateTime(editableModel?.cancelledDate)}</span> {t('stockmanagement.by')} <span className='action-by'>{editableModel.cancelledByFamilyName ?? ""} {editableModel.cancelledByGivenName ?? ""}</span>
...

              {
                ...
                <div className='field-label'>
                  <span className='field-title'>{t('stockmanagement.rejected')}:</span>
                  <span className='field-desc'>
                    <span className='action-date'>{formatDisplayDateTime(editableModel?.rejectedDate)}</span> {t('stockmanagement.by')} <span className='action-by'>{editableModel.rejectedByFamilyName ?? ""} {editableModel.rejectedByGivenName ?? ""}</span>
            ...
            {((!canEdit && (canApprove || canReceiveItems)) || (!isNew && (canEdit || canPrint)) || isRequisitionAndCanIssueStock) &&
              <div className='right-actions'>
                {!canEdit && canApprove &&
                  <>
                    {!requiresDispatchAcknowledgement && <Button name="approve" type="submit" size="sm" className="submitButton" onClick={handleApprove} kind="primary" renderIcon={CheckmarkOutline24}>{t("stockmanagement.approve")}</Button>}
                    {requiresDispatchAcknowledgement && <Button name="approveDispatch" type="submit" size="sm" className="submitButton" onClick={handleApprovalDispatch} kind="primary" renderIcon={Departure24}>{t("stockmanagement.dispatch")}</Button>}
                    <Button name="reject" type="submit" size="sm" className="submitButton" onClick={handleReject} kind="danger--primary" renderIcon={CloseOutline24}>{t("stockmanagement.reject")}</Button>
                    <Button name="return" type="button" size="sm" className="cancelButton" kind="tertiary" onClick={handleReturn} renderIcon={Repeat24}>{t("stockmanagement.return")}</Button>
                    <Button name="cancel" type="button" size="sm" className="cancelButton" kind="danger--ghost" onClick={handleCancel} renderIcon={Error24}>{t("stockmanagement.canceltransaction")}</Button>
                  </>
                }
                {!canEdit && canReceiveItems &&
                  <>
                    <Button name="completedispatch" type="submit" size="sm" className="submitButton" onClick={handleCompleteDispatch} kind="primary" renderIcon={Arrival24}>{t("stockmanagement.stockoperation.edit.complete")}</Button>
                    <Button name="return" type="button" size="sm" className="cancelButton" kind="tertiary" onClick={handleReturn} renderIcon={Repeat24}>{t("stockmanagement.return")}</Button>
                  </>
                }
                {!isNew && canEdit &&
                  <Button type="button" size="sm" className="cancelButton" kind="danger--tertiary" onClick={handleCancel} renderIcon={Error24}>{t("stockmanagement.canceltransaction")}</Button>
                }
                {
                  isRequisitionAndCanIssueStock &&
                  <>
                    <Button type="button" size="sm" className="cancelButton" kind="tertiary" onClick={handleIssueStock} renderIcon={DeliveryTruck24}>{t("stockmanagement.issuestock")}</Button>
                  </>
                }
                {
                  !isNew && canPrint && <>
                    <Button type="button" size="sm" className="cancelButton" iconDescription={"Print"} kind="ghost" renderIcon={Printer24} onClick={onPrintStockOperation}>{t("stockmanagement.print")}</Button>
                  </>
                }
              </div>
            }
        ...
        {
          fetchedStockOperationLinks && stockOperationLinks && stockOperationLinks.results?.length > 0 &&
            ...
            <h6>Related Transactions:</h6>
            {
              stockOperationLinks.results?.map(p =>
                <div key={p.uuid} className="stkpg-rel-op-item">
                  {(editableModel?.uuid === p.parentUuid || requisitionUuid === p.parentUuid) && <>
                    <span>{p.childOperationTypeName}</span>
                    <span className={p.childVoided ? "voided" : ""}>
                      {p.childVoided && p.childOperationNumber}
                      {!p.childVoided && <Link target={'_blank'} to={URL_STOCK_OPERATION(p.childUuid)}>{p.childOperationNumber}</Link>}
                    </span>
                    <span>[{p.childStatus}]</span>
                    <OverflowMenuVertical24 />
                  </>
                  }
                  {(editableModel?.uuid === p.childUuid || requisitionUuid === p.childUuid) && <>
                    <span>{p.parentOperationTypeName}</span>
                    <span className={p.parentVoided ? "voided" : ""}>
                      {p.parentVoided && p.parentOperationNumber}
                      {!p.parentVoided && <Link target={'_blank'} to={URL_STOCK_OPERATION(p.parentUuid)}>{p.parentOperationNumber}</Link>}
                    </span>
        ...
        <Tabs selected={selectedTab} type="container" onSelectionChange={(e) => setSelectedTab(e)}>
          <Tab label={`${stockOperationType?.name ?? ""} Details`}>
            <EditStockOperation model={editableModel!} setModel={setEditableMode}
              isNew={isNew}
          ...
          <Tab label="Stock Items" disabled={!showItems}>
            <StockOperationItemsTable setStockOperationItems={setStockOperationItems} addItem={addNewStockOperationItem}
              items={stockOperationItems} canEdit={canEdit} setShowComplete={setShowComplete}
          ...
          <Tab label={requiresDispatchAcknowledgement ? "Submit/Dispatch" : "Submit/Complete"} disabled={!showItems || !showComplete} >
            <SubmitApproval
              ...
              canEdit={canEdit}
              requiresDispatchAcknowledgement={requiresDispatchAcknowledgement}
              locked={editableModel?.locked ?? false}
              currentStockOperationType={stockOperationType}
              isNew={isNew}
              actions={{ onSave: handleSave, onGoBack: handleGoBack, onComplete: handleComplete, onSubmit: handleSubmit, onDispatch: handleDispatch }} />
          </Tab>
          {
            canDisplayReceivedItems &&
            <Tab label={"Received Items"} disabled={!showItems || !showComplete}>
              <ReceiveItemsTable setStockOperationItems={setStockOperationItems}
                items={stockOperationItems} canReceiveItems={canReceiveItems}
                isNegativeQtyAllowed={isNegativeQtyAllowed}
                locked={editableModel?.locked ?? false}
                actions={{ onGoBack: handleGoBack, onSaveReceivedItems: handleSaveReceivedItems }}
